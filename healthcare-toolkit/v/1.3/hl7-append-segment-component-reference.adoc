...
*Append Segment component*将段添加到HL7消息。像 link:/healthcare-toolkit/v/1.3/hl7-delete-segment-component-reference[HL7删除段组件]一样，它可以处理HAPI消息对象的实例。如果您的HL7消息不是HAPI消息对象，则必须使用 link:/healthcare-toolkit/v/1.3/hl7-encoding-transformer[HL7编码变压器]在通过附加段组件之前对其进行转换。

[NOTE]
HL7追加段组件是 link:/healthcare-toolkit/v/1.3[Mule医疗工具包]的一部分，该组件包括用于接收，处理和发送HL7消息的变换器，组件和连接器。有关可用工具的详细信息和完整列表，请参阅工具包的 link:/healthcare-toolkit/v/1.3[主要文档页面]。
...
[NOTE]
如果您希望修改的HL7消息不是HAPI对象，请将其转换为带 link:/healthcare-toolkit/v/1.3/hl7-encoding-transformer[HL7编码变压器]的HAPI对象，您可以将它立即放置在追加段组件之前。
