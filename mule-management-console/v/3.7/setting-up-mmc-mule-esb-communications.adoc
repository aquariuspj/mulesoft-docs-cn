...
本文档假定您熟悉基本的 link:/mule-management-console/v/3.7/architecture-of-the-mule-management-console[Mule管理控制台的架构]，并且您拥有 link:/mule-management-console/v/3.7/installing-mmc[安装]管理控制台的试用版或生产版。
...
当您第一次运行MMC的 link:/mule-management-console/v/3.7/installing-the-production-version-of-mmc[生产版本]时，它将尝试检测网络上的任何Mule ESB服务器，但不会尝试与其找到的任何服务器配对。您必须手动注册您的Mule ESB服务器。
