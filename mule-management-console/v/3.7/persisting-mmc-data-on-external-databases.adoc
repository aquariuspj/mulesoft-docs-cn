...
[NOTE]
此页面和此处链接的大多数页面旨在配置_fresh MMC安装上的数据持久性_，即尚未配置为将数据保存到外部数据库的安装。如果您的MMC安装已设置为将数据保存到外部数据库，并且您希望升级到MMC 3.5.0，则需要执行特殊步骤，包括迁移旧配置数据。要确定要升级的确切过程，请参阅 link:/mule-management-console/v/3.7/upgrading-the-management-console[升级管理控制台]。
...
*  link:/mule-management-console/v/3.7/persisting-mmc-data-to-ms-sql-server[MS SQL Server] _ _
