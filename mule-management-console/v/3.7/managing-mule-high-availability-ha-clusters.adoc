...
[NOTE]
有关群集的概述（包括收益和最佳做法），请参阅 link:/mule-user-guide/v/3.7/mule-high-availability-ha-clusters[Mule高可用性HA集群]。

您可以使用控制台执行以下与集群相关的任务：

*  link:/mule-management-console/v/3.7/creating-or-disbanding-a-cluster[创建一个集群]
*  link:/mule-management-console/v/3.7/deploying-redeploying-or-undeploying-an-application-to-or-from-a-cluster[将应用程序部署，重新部署或从群集中取消部署]
*  link:/mule-management-console/v/3.7/monitoring-a-cluster[监视群集，包括其状态]
