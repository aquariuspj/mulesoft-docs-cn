...
[WARNING]
本文档介绍了如何使用Apache Tomcat将生产环境部署到管理控制台。所有说明也应与其他 link:/mule-user-guide/v/3.6/hardware-and-software-requirements[支持的应用服务器]一起使用，但尚未经过测试。
...
关于SSL配置，有一个 link:http://bugs.java.com/bugdatabase/view_bug.do?bug_id=7105007[已知问题]，其中涉及SSL配置，导致数据库连接尝试无限期地挂起，没有超时，从而阻止MMC启动。 MS SQL不能用于此Java版本。请升级到更高版本的JVM。
====
...
有关更多信息，请参阅 link:http://www-01.ibm.com/support/knowledgecenter/SS7JFU_7.0.0/com.ibm.websphere.express.doc/info/exp/ae/rdat_minreqmssql.html[数据源Microsoft SQL Server的最低要求设置]的IBM文档。
====
