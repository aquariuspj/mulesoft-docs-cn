...
本页面介绍通过管理控制台设置用户的基本知识。有关设置用户组和分配权限的信息，请参阅 link:/mule-management-console/v/3.6/managing-mmc-users-and-roles[管理MMC用户和角色]。
...
[NOTE]
====
默认组是管理员，部署者，监视器和服务器管理员。如果您需要创建更多组或角色，请参阅 link:/mule-management-console/v/3.6/managing-mmc-users-and-roles[管理MMC用户和角色]。
====
