...
本文假定您有 link:https://www.mulesoft.com/ty/dl/studio[下载并安装Anypoint Studio]。如果您之前没有使用Eclipse或基于Eclipse的IDE的经验，请查看 link:/anypoint-studio/v/6.5/index[Anypoint Studio界面]的简要介绍。该文档还假定您熟悉 link:https://maven.apache.org/[Apache Maven]并知道它的用途。
...
----
<1>这个类扩展了FunctionalTestCase，这是Mule抽象JUnit测试用例的实现。有关更多信息，请参阅 link:/mule-user-guide/v/3.8/functional-testing[功能测试]。
<2>创建一个新的Mule客户端。
<3>使用HTTP GET方法向客户端发送HTTP URL。
<4>声明返回值不为null并且等于"Hello World!"。
...
这将使用您的Mule版本所需的所有模块填充您的本地Maven存储库（在Linux中，通常位于$ HOME / .m2 / repository /中）。控制台会通知您导入过程的进度。

== 另请参阅

* 详细了解 link:/mule-user-guide/v/3.8/using-maven-with-mule[和Mule一起使用Maven]。

* 了解如何 link:/anypoint-studio/v/6.5/maven-support-in-anypoint-studio[配置您的Studio首选项以使用Maven]。
* 了解如何 link:/anypoint-studio/v/6.5/enabling-maven-support-for-a-studio-project[“mavenize”现有的Anypoint Studio项目]。

* 了解如何 link:/anypoint-studio/v/6.5/importing-a-maven-project-into-studio[将现有的Maven项目导入Anypoint Studio]。

* 访问其他Maven link:/mule-user-guide/v/3.8/maven-reference[参考]和 link:/mule-user-guide/v/3.8/configuring-maven-to-work-with-mule-esb[组态]信息。
