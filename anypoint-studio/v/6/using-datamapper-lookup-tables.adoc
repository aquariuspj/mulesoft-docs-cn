= 使用DataMapper查找表（不推荐使用--DataMapper）
:keywords: datamapper
...
Anypoint DataMapper支持 link:/anypoint-studio/v/6/building-a-mapping-flow-in-the-graphical-mapping-editor[查找表]，这有助于从各种来源检索数据以用于元素映射。例如，您可以使用查找表来提供地理数据等参考数据，检索用于映射的客户数据，甚至可以调用另一个返回某个业务流程结果的Mule流。
