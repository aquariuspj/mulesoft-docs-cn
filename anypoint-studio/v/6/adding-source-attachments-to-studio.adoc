...
使用 link:/anypoint-studio/v/6/using-maven-in-anypoint-studio[Maven在Anypoint Studio中]作为项目管理工具时，您可以选择下载所有源的`.jar`文件以满足项目的直接和传递依赖关系。 Studio意识到这一点，并在可能的情况下查看本地Maven存储库中的源代码。通过选中标记为"Download sources when updating dependencies"的复选框，您可以在更新Maven设置首选项页面中的依赖关系时打开下载源的功能。
...
*  link:/anypoint-studio/v/6/studio-visual-debugger[Studio可视化调试器]
