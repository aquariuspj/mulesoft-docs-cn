...
如果您不使用Anypoint Studio进行开发，请按照 link:http://mulesoft.github.io/servicenow-connector/4.0.1/mule/servicenow-config.html[安装ServiceNow Maven依赖关系]中的说明进入您的`pom.xml`文件。
...
请联系mailto：info@mulesoft.com [MuleSoft销售团队]获取其中任何一个。 （详细了解 link:/mule-user-guide/v/3.8/installing-an-enterprise-license[安装企业许可证]。）

== 配置

要在Mule应用程序中使用ServiceNow连接器，您必须配置：

* 全局ServiceNow元素将被应用程序中的所有ServiceNow连接器使用（请阅读有关 link:/mule-user-guide/v/3.8/global-elements[全球元素在骡子]的更多信息）。
