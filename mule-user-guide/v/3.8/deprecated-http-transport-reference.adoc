...
您可以使用 link:/mule-user-guide/v/3.8/https-transport-reference[HTTPS传输参考]通过HTTP创建安全连接。如果要保护对HTTP端点的请求，则HTTP连接器支持HTTP Basic / Digest身份验证方法（以及Mule通用头身份验证）。要配置HTTP Basic，请在HTTP端点上配置 link:/mule-user-guide/v/3.8/configuring-security[安全端点过滤器]。
...
您必须在安全过滤器认证的Mule实例上配置安全管理器。有关安全配置选项和示例的信息，请参阅 link:/mule-user-guide/v/3.8/configuring-security[配置安全性]。有关端点配置的一般信息，请参阅 link:/mule-user-guide/v/3.8/endpoint-configuration-reference[端点配置参考]。
...
该连接器还接受来自 link:/mule-user-guide/v/3.8/tcp-transport-reference[TCP连接器]的所有属性。
...
HTTP入站端点属性将覆盖为 link:/mule-user-guide/v/3.8/endpoint-configuration-reference[默认入站端点属性]指定的属性。
...
HTTP出站端点属性将覆盖为 link:/mule-user-guide/v/3.8/endpoint-configuration-reference[默认出站端点属性]指定的属性。
...
HTTP端点属性会覆盖为 link:/mule-user-guide/v/3.8/endpoint-configuration-reference[默认的全局端点属性]指定的属性。
...
* 了解如何 link:/mule-user-guide/v/3.8/migrating-to-the-new-http-connector[迁移到新的HTTP连接器]
* 获取有关新 link:/mule-user-guide/v/3.8/http-connector[HTTP连接器]的概述
* 查看有关新 link:/mule-user-guide/v/3.8/http-connector-reference[HTTP连接器参考]的参考信息
