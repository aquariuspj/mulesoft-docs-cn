...
| jco.client.snc_mode  |安全网络连接（SNC）模式，0 image:lightbulb.png[（灰色灯泡）]或1 image:lightbulb-on.png[（灯泡上）]  |  |
...
| jco.destination.one_roundtrip_repository  | 1在SAP Server中强制使用RFC_METADTA_GET，0取消激活它。如果未设置，则目标将最初是远程调用，以检查RFC_METADATA_GET是否可用。 |  |
|===
...
要获得更新和完整的列表，请查看您的Jco发行版中的com.sap.conn.jco.ext.ServerDataProvider的Javadocs。
