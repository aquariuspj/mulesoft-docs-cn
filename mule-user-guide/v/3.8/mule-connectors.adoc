...
link:/mule-user-guide/v/3.8/anypoint-connectors[Anypoint连接器]在Mule和一个或多个外部来源（如文件，数据库或Web服务）之间接收或发送消息。连接器可以充当入站端点作为消息源，它们可以充当消息处理器，在流的中间执行操作，也可以在流的末尾并充当最终有效负载数据的接收者。
...
*  *NEXT STEP*：请阅读大约 link:/mule-user-guide/v/3.8/mule-components[组件]。
