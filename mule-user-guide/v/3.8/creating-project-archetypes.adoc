...
[TIP]
要使用API​​Kit原型创建APIKit项目，请检查 link:/apikit/creating-an-apikit-project-with-maven[使用Maven创建APIKit项目]。

== 配置Maven

将以下内容添加到文件`settings.xml`（通常在您的Maven `$HOME/.m2`或`conf`目录中，例如`/usr/local/apache-maven/apache-maven-<version>/conf`），以便Maven允许您执行Mule插件。
