...
当发生系统异常时，Mule向注册的侦听器发送异常通知，记录异常，并且 - 如果异常是由连接失败引起的 - 则执行 link:/mule-user-guide/v/3.8/configuring-reconnection-strategies[重新连接策略]。系统异常策略不能在Mule中配置。
...
* 无论您在Flow级别定义什么， link:/mule-user-guide/v/3.8/transaction-management[交易范围]都可能包含自己的异常策略。
...
*  link:/mule-user-guide/v/3.8/mule-message-structure[骡信息]。
