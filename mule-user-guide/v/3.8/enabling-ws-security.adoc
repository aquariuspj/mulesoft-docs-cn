...
当Web服务将私有网络数据公开给外部世界时，该数据通过4-7个单独的协议层（参见： link:http://en.wikipedia.org/wiki/TCP/IP_model[TCP / IP]或 link:http://en.wikipedia.org/wiki/OSI_model[OSI]）传播，因此引入了潜在的安全漏洞。例如，在协议栈上的较高层实现安全性（例如应用层）通常提供比在较低传输层（仅提供HTTPS安全性）的实现更好的保护。
...
对于涉及Web服务的Mule流，添加 link:/mule-user-guide/v/3.8/cxf-component-reference[CXF组件]以发布，使用或代理Web服务。完成以下步骤以在SOAP组件上配置WS-security。
...
. Search for "cxf" and drag the CXF (SOAP) processor to the canvas.
...
*  link:/mule-user-guide/v/3.8/cxf-component-reference[CXF组件]
