...
使用*session variable*在整个生命周期中设置或删除与消息绑定的变量，可能跨多个流，应用程序甚至服务器。会话变量很容易通过VM传输或流引用从一个流传播到另一个流，但不能通过 link:/mule-user-guide/v/3.8/http-connector[HTTP连接器]传播。相比之下，通过 link:/mule-user-guide/v/3.8/variable-transformer-reference[可变变压器]指定的变量只要与它们关联的消息保持在同一流程中，就会一直存在。要详细了解消息范围，请参阅 link:/mule-user-guide/v/3.8/mule-concepts[骡子概念]。
...
| *Persistence*：使用会话变量变换器设置的会话变量会保留整个消息生命周期，而不管传输障碍如何， link:/mule-user-guide/v/3.8/http-connector[HTTP连接器]不会传播它们。{{5} } *Persistence*：使用变量变量设置的变量仅保留当前流并且不能穿过传输障碍。 | *Persistence*：一旦消息遇到出站连接器，出站范围与特定于传输的元数据（例如，HTTP出站连接器的HTTP头）一起以消息的形式发送。
|===
...
[WARNING]
请注意，会话变量链接到Mule消息，但不链接到MuleEvent。这意味着，如果您使用了复制消息的元素（例如 link:/mule-user-guide/v/3.8/async-scope-reference[异步范围]或 link:/mule-user-guide/v/3.8/scatter-gather[分散收集组件]），那么流变量的更改将不会在这些复制消息之间持续存在，即使他们从一开始。
...
* 阅读相关转换器 link:/mule-user-guide/v/3.8/variable-transformer-reference[可变变压器]和 link:/mule-user-guide/v/3.8/property-transformer-reference[属性变压器]，您可以使用它们为不同范围设置属性和变量。
