...
[TIP]
====
目前， link:http://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/component/BindingInvocationHandler.html[org.mule.routing.binding.BindingInvocationHandler]的`invoke()`方法在出现异常时总是抛出`UndeclaredThrowableException`，即使绑定的方法/接口抛出其中一个声明的异常。从Mule 3.x开始，如果原因已知并且类型与给定方法的"throws"子句中声明的某个异常相匹配，则将返回声明的异常。
====
