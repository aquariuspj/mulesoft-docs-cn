...
* 在使用共享资源的应用程序的配置文件中添加属性可能会导致问题，因为这些属性与域中的其他应用程序共享，并且可能存在冲突。您可以改为 link:/mule-user-guide/v/3.8/setting-environment-variables[设置环境变量]。
...
| *mule-deploy.properties*  | 该可选文件可能包含两个配置属性，与Mule应用程序中的配置属性相同：*redeployment.enabled*和*loader.overrides*。有关更多详情，请参阅 link:/mule-user-guide/v/3.8/mule-application-deployment-descriptor#deployment-descriptor-in-a-domain[部署描述符在一个域中]。
|===
...
共享 link:/mule-user-guide/v/3.8/tls-configuration[TLS配置]允许共享<<HTTP,listener configurations>>使用相同的TLS配置。 +
...
* 请注意，虽然共享资源仅限于本文档中所选的连接器和库，但在Mule中可以共享其他配置片段。有关详细信息，请参阅 link:/mule-user-guide/v/3.8/sharing-custom-configuration-fragments[共享自定义配置片段]。
...
*  link:/mule-user-guide/v/3.8/database-connector[D B]
...
*  link:/mule-user-guide/v/3.8/jboss-transaction-manager-reference[JBoss的]
