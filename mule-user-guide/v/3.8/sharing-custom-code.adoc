...
* 自定义变压器 - 执行任何Mule股票转换器都无法执行的操作（请参阅 link:/mule-user-guide/v/3.8/creating-custom-transformers[这里]）。
* 自定义组件 - 通常是支持Mule的组件或非面向业务的组件，因为业务组件通常是来自预先存在的项目的简单POJO（请参阅 link:/mule-user-guide/v/3.8/developing-components[这里]）。
