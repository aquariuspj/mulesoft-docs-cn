...
在版本3.5.0之前，Mule使用JBoss事务管理器来配置 link:/mule-user-guide/v/3.8/xa-transactions[XA交易]。从版本3.5.0开始， link:https://github.com/bitronix/btm[Bitronix]是Mule Enterprise的推荐事务管理器，因为它提供了开箱即用的支持XA事务恢复。
...
本文假定您熟悉Mule中的 link:/mule-user-guide/v/3.8/shared-resources[共享资源]。
...
要在与Mule域相关的所有应用程序中使用Bitronix，请在您的`mule-domain-config.xml`中将Bitronix定义为交易管理器，如下所示。这样做会导致与该域关联的所有应用程序使用Bitronix，而无需在单个应用程序中声明事务管理器。有关域配置的更多详细信息，请参阅 link:/mule-user-guide/v/3.8/shared-resources[共享资源。]
...
Bitronix集成为 link:/mule-user-guide/v/3.8/database-connector[数据库连接器]提供了一个数据源连接池，只要您想要在数据库连接器中实现资源池，就必须使用它。
...
* 详细了解如何操作
link:/mule-user-guide/v/3.8/shared-resources[将Bitronix事务管理器定义为共享资源]为与域关联的多个应用程序。
