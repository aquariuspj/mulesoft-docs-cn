...
安全管理器可以配置一个或多个加密策略，然后可以使用加密转换器，安全过滤器或安全传输（例如 link:/mule-user-guide/v/3.8/ssl-and-tls-transports-reference[SSL]或 link:/mule-user-guide/v/3.8/http-connector[HTTPS]）。这些加密策略可以大大简化安全消息传递的配置，因为它们可以跨组件共享。
