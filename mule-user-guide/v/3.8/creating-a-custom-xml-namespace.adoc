...
其他名称空间可以通过`<spring:beans>`或通过在模块中添加专用元素来引入（请参阅 link:/mule-user-guide/v/3.8/scripting-module-reference[脚本模块的] `<lang>`元素）。
...
* 有关可变和可扩展容器的文章： link:http://www.xfront.com/VariableContentContainers.pdf[VariableContentContainers]和 link:http://www.xfront.com/ExtensibleContentModels.pdf[ExtensibleContentModels]
