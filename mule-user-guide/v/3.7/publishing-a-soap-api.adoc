...
Mule利用Apache的开源 link:http://cxf.apache.org/docs/a-simple-jax-ws-service.html[CXF框架]来构建API。您可以在Studio中创建CXF API，方法是在您的Mule流中配置CXF组件，以执行与发布SOAP API相关的以下任何CXF操作：

* 发布一个简单的服务
* 发布JAX-WS服务
* 代理已发布的服务

使用Mule的CXF组件，您还可以 link:/mule-user-guide/v/3.7/securing-a-soap-api[启用WS安全]，指定数据绑定，并为CXF API添加拦截器。有关Mule如何利用CXF框架的更多信息，请参阅 link:/mule-user-guide/v/3.7/cxf-module-reference[CXF模块配置参考]。



== 假设

本文档在Anypoint Studio，Mule ESB的图形用户界面（GUI）的上下文中描述了示例的细节。在适当的情况下，XML配置伴随着Studio界面截图。本文档假定您熟悉Mule ESB和 link:/anypoint-studio/v/5/index[Anypoint Studio界面]。
...
通过CXF，Mule允许您使用两个 link:https://en.wikipedia.org/wiki/Web_service#Automated_design_methods[方法]中的一个来呈现此WSDL合约：
...
*_Enterprise_* +
为了保护Mule流中的SOAP API，您可以配置CXF组件中的元素以应用WS安全性。您可以添加 link:http://cxf.apache.org/docs/ws-security.html[WS-Security的] *Configuration Elements*（即键值对）来验证和/或验证消息的SOAP标头中的信息;您还可以启用六个*Token Validators*中的一个或多个以确保消息安全。添加这些安全功能的功能仅在企业版Mule中可用。

请参阅 link:/mule-user-guide/v/3.7/securing-a-soap-api[保护SOAP API]文档以配置API的安全元素。

== 另请参阅

* 请参阅 link:/mule-user-guide/v/3.7/cxf-component-reference[CXF组件参考]。
...
* 了解将您的Java类变为WSDL所需的 link:http://en.wikipedia.org/wiki/Java_API_for_XML_Web_Services[JAX-WS]注释。
