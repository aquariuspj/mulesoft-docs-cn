...
有关为端点功能或操作配置连接器的说明，请参阅 link:/mule-user-guide/v/3.7/connector-configuration-reference[连接器配置参考]。

有关此基于操作的连接器的信息，请参阅 link:/mule-user-guide/v/3.7/http-connector[HTTP连接器]。
...
基于操作的连接器需要全局连接器配置（通常对基于端点的连接器是可选的）来指定连接参数，例如用户名，密码和安全令牌。其他全局参数也可以配置。有关详细信息，请参阅每个连接器的单独参考。一般说明可在 link:/mule-user-guide/v/3.7/connector-configuration-reference[连接器配置参考]上找到。
