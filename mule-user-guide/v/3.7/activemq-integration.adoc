...
使用https://repository.apache.org/content/repositories/releases/org/apache/activemq/activemq-all/[activemq-all-x.x.x.jar]会与其他依赖关系产生冲突，最明显的是与log4j冲突。只添加您为ActiveMQ选择的部署模式所需的JAR文件：
...
您也可以使用其他连接器属性（有关更多详细信息，请参阅“配置参考”）。
...
达到限制时，除非客户配置了 http://activemq.apache.org/message-redelivery-and-dlq-handling.html[DLQ（死信队列）]，否则该消息将自动丢弃。
====
