...
在Mule中，表达式是一个字符串，由`\#[ ]`包围。在运行时，Mule使用MEL（作为默认表达式计算器）来确定表达式的值。 Mule还支持其他类型的表达式（它也必须包含在`\#[ ]`中），它们用于评估表达式，如果它不能被识别为MEL。
...
[NOTE]
====
请注意，MEL是*expression language*，而不是*programming language*。因此，Mule中的 link:http://en.wikipedia.org/wiki/Expression_%28computer_science%29[表达]：

* 始终解析为单个值
*  _ cannot_有一个void返回类型
====
...
实际上，没有一个单一的_typical_ MEL表达式。** **也就是说，一些示例表达式可以帮助说明MEL表达式如何解析。如下面的示例表格所示，MEL表达式返回的值可以是数值，逻辑值（`true`或`false`），字符串或几乎任何数据类型。 MEL表达式也可以执行操作，调用方法和执行函数。查看完整的 link:/mule-user-guide/v/3.7/mule-expression-language-basic-syntax[语法参考]，探索所有可能性。访问完整的 link:/mule-user-guide/v/3.7/mule-expression-language-examples[例子]，演示如何在应用程序中使用MEL表达式。
...
[TIP]
有关日期和时间函数的完整参考，请参阅 link:/mule-user-guide/v/3.7/mule-expression-language-date-and-time-functions[Mule表达语言日期和时间函数]。
