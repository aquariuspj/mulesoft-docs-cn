...
通常情况下，您可以将它放在另一个消息处理器（如连接器）之前，以确定是否应处理传入消息或事件，从而允许消息只在满足过滤器验证时沿着流继续，并且包含的​​XML被认为是有效的。
...
为了允许有效的XML通过过滤器，您需要在架构验证过滤器中将`returnResult`设置为false。通过这种方式，向前传递到流中下一个元素的消息的有效内容将保留XML内容。
...
在上面的示例中，如果XML内容与XSD验证不匹配，则会将消息导向名为`error_invalid_data`的流。
