...
您也可以考虑将业务逻辑建模为 link:/mule-user-guide/v/3.7/bpm-module-reference[处理]，具体取决于涉及的算法的性质。
...
[NOTE]
====
由于它们密切相关，因此BPM命名空间/模式在Drools模块和 link:/mule-user-guide/v/3.7/bpm-module-reference[BPM模块]之间共享。
====
...
[NOTE]
====
就像 link:/mule-user-guide/v/3.7/bpm-module-reference[BPM]一样，Mule中的规则集成基于可插入的接口，因此可以使用任何实现此接口的Java规则引擎来代替Drools。然而，目前唯一可用的这种实现是Drools。
====
