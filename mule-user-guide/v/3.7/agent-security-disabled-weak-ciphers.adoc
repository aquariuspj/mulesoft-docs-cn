...
代理程序接收来自 link:/mule-management-console/v/3.7[Mule管理控制台]的连接请求，这是您可以用来监视和控制Mule ESB服务器和应用程序的图形工具。出于安全原因，从3.3.1开始，Mule代理拒绝使用弱密码的连接。即使代理只接受来自授权管理控制台的连接，如果环境受到安全审核（如 link:http://www.pcicomplianceguide.org/aboutpcicompliance.php[PCI合规性]的自动审核），禁用的弱密码也是非常有用的功能。
