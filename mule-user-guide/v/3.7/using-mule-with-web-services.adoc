...
*  link:/mule-user-guide/v/3.7/proxying-web-services[为现有Web服务创建代理]（例如，向现有Web服务添加安全性）

在这里，我们更深入地探讨这些场景中存在哪些选项，并指导您参考其他文档资源和示例。

==  CXF模块概述

在解释使用Mule ESB可以完成的不同Web服务场景之前，重要的是要明白它所依赖的主要技术是 link:http://cxf.apache.org/[Apache CXF]。 Apache CXF是一个开源服务框架，可帮助您使用前端编程API（如JAX-WS和JAX-RS）构建和开发服务。 Mule ESB现在捆绑提供Web服务集成的 link:/mule-user-guide/v/3.7/cxf-module-reference[CXF模块]，以消耗现有的Web服务并构建新的服务。

[TIP]
从最新版本的Mule ESB开始，推荐使用 link:/mule-user-guide/v/3.7/cxf-module-configuration-reference[CXF模块]来处理Web服务。

== 使用Web服务

如果调用现有的Web服务，通​​常会构建一个Web服务客户端来执行调用。如果您曾经体验过这一点，您会理解它需要一些代码生成 - 您可以通过Apache CXF link:http://cxf.apache.org/docs/how-do-i-develop-a-client.html[这里]了解不同的方法来完成此操作）。
...
. 使用CXF中的 link:https://cwiki.apache.org/confluence/display/CXF20DOC/WSDL+to+Java[WSDL到Java]工具或 link:https://cwiki.apache.org/confluence/pages/viewpage.action?pageId=2853210[Maven插件]生成CXF客户端。
. 使用`<cxf:jaxws-client>`组件在Mule配置XML文件中配置客户端。
. 在Mule配置XML文件中配置一个终端，该终端将作为请求服务的传输（通常是HTTP）。

有关更多信息，可以查看 link:/mule-user-guide/v/3.7/consuming-web-services-with-cxf[使用CXF消费Web服务]文档。
...
有关更多信息，请阅读 link:/mule-user-guide/v/3.7/consuming-web-services-with-cxf[使用CXF消费Web服务]文档。
...
*  link:/mule-user-guide/v/3.7/consuming-web-services-with-cxf[使用JAX-WS客户端API]。
*  link:/mule-user-guide/v/3.7/consuming-web-services-with-cxf[构建客户端代理]。
...
有关更多信息，请阅读 link:/mule-user-guide/v/3.7/building-web-services-with-cxf[创建一个JAX-WS服务]文档。

=== 创建一个WSDL第一个JAX-WS服务

这是您首先定义WSDL然后编写Web服务的地方。要以这种方式构建Web服务并在Mule中配置它，您应该：

. 使用CXF中的 link:http://cxf.apache.org/docs/wsdl-to-java.html[WSDL到Java]工具或 http://cxf.apache.org/docs/maven-cxf-codegen-plugin-wsdl-to-java.html[Maven插件]从WSDL生成Web服务接口。
. 编写实现您的服务接口的服务实现类。
. 配置将接受对Web服务的请求的端点，通常是HTTP入站端点。
. 使用`<cxf:jaxws-service>`组件在Mule XML配置文件中配置服务。

有关更多信息，请阅读 link:/mule-user-guide/v/3.7/building-web-services-with-cxf[创建一个WSDL第一个JAX-WS服务]文档。
...
有关更多信息，请阅读 link:/mule-user-guide/v/3.7/building-web-services-with-cxf[创建一个简单的前端Web服务]文档。
...
*  link:/mule-user-guide/v/3.7/proxying-web-services-with-cxf[服务器端代理]
*  link:/mule-user-guide/v/3.7/proxying-web-services-with-cxf[客户端代理]
