...
当Web服务向外界公开专用网络数据时，该数据将通过4  -  7个独立的协议层（请参阅 link:http://en.wikipedia.org/wiki/TCP/IP_model[TCP / IP]或 link:http://en.wikipedia.org/wiki/OSI_model[OSI]）传播，从而引发潜在的安全漏洞。例如，在协议栈上的较高层实现安全性（例如应用层）通常提供比在较低传输层（仅提供HTTPS安全性）的实现更好的保护。

有关更多信息，请参阅Anypoint Exchange中的https://www.anypoint.mulesoft.com/exchange/?search=soap%20security[SOAP Web Service安全性]。您还可以使用 link:/mule-user-guide/v/3.7/web-service-consumer[Web服务使用者]为WS-Security提供用户名令牌和时间戳支持。

要实现应用程序层安全性，请在Web服务上启用 link:http://msdn.microsoft.com/en-us/library/ms977327.aspx[WS-安全]（ link:http://cxf.apache.org/[CXF]配置）。 WS-security定义了一个新的SOAP头，它能够携带各种安全令牌，系统用它来标识Web服务调用者的身份和特权，并且可以包含以下内容：

* 加密
* 数字签名
* 时间戳

您还可以将xref：addtokenvalidator [添加自定义令牌验证程序]添加到SOAP组件。
...
对于涉及Web服务的Mule流，添加 link:/mule-user-guide/v/3.7/cxf-component-reference[CXF组件]以发布，使用或代理Web服务。完成以下步骤以在SOAP组件上配置WS-security。
...
. To your CXF element (that is, a SOAP component), add a child element for `cxf:ws-security`.
...
link:http://www.mulesoft.org/schema/mule/cxf/3.7/mule-cxf.xsd[查看Mule 3.7 CXF架构]。
...
* 请参阅https://www.anypoint.mulesoft.com/exchange/?type=example[examples]。
