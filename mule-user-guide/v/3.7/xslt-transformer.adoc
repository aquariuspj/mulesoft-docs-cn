...
Mule 3.6.0和更高版本支持XSLT转换器的XSLT版本3.0。早期版本中变压器的行为和语法保持不变。您可以选择使用`version`声明的变压器将使用的XSLT版本（请参见下文）。有关Mule中的XML，XSLT，Xquery和XPath 3.0支持的详细信息，请参阅 link:/mule-user-guide/v/3.7/xml-module-reference[XML模块参考]。
====
...
该变压器可以使用以下功能测试进行测试。请注意，它使用`FunctionalTestCase`，这是Mule的 link:/mule-user-guide/v/3.7/functional-testing[测试支持]的一部分。
