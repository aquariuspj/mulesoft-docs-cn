...
当Web服务向外界公开私人网络数据时，该数据通过4-7个独立的协议层传播，因此引入了潜在的安全漏洞。例如，在协议栈上的较高层实现安全性（例如应用层）通常提供比在较低传输层（仅提供HTTPS安全性）的实现更好的保护。
...
您还可以将自定义 link:/mule-user-guide/v/3.3/enabling-ws-security[*令牌验证器*]添加到SOAP组件。

== 启用WS-Security

要在应用程序中的CXF Web服务上启用WS安全性，您必须将WS配置添加到流中的*SOAP Component*。此方法仅在一个Web服务中启用流程中的本地WS安全性。

对于涉及Web服务的Mule流，添加 link:/mule-user-guide/v/3.3/soap-component-reference[* SOAP组件*]以发布，使用或代理Web服务。完成以下步骤以在SOAP组件上配置WS-security。
...
* 在`CallbackHandler`实现无法访问密码的情况下验证UsernameToken
* 验证接收到的SAML断言的属性
* 将收到的安全令牌分派给第三方安全服务进行验证
...
*  link:/mule-user-guide/v/3.3/soap-component-reference[SOAP组件]
