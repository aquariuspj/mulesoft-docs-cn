...
Mule有能力使用 http://www.pgpi.org/doc[相当好的隐私（PGP）]加密消息有效负载或有效负载的一部分。 PGP结合了数据压缩和数据加密来保护消息。压缩减少了原始和目的地之间的消息传输时间，并增加了密码安全性。
...
* 了解如何使用 link:/mule-user-guide/v/3.3/mule-credentials-vault[Mule证书库]加密您的属性文件。
* 了解如何使用 link:/mule-user-guide/v/3.3/mule-message-encryption-processor[XML或JCE加密]加密或解密邮件。
