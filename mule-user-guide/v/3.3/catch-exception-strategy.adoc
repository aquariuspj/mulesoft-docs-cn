...
* 避免将例外传播到入站端点
* 避免通过流参考组件向父流传播异常
* 确保由流处理的事务在发生错误时不会回退（即事务永远不会“回滚”以重新尝试处理; Mule会提交事务。）
...
您可以创建一个或多个 link:/mule-user-guide/v/3.3/error-handling[全球例外策略]，以便在整个Mule应用程序的流程中重复使用。首先，创建一个全局捕获异常策略，然后向流中添加 link:/mule-user-guide/v/3.3/reference-exception-strategy[*参考例外战略*]以应用新的全局捕获异常策略的错误处理行为。
...
使用[引用异常策略]指示流程采用由您的全局捕获异常策略定义的错误处理行为。换句话说，您必须要求您的流程参考全局捕获异常策略以获取有关如何处理错误的说明。
