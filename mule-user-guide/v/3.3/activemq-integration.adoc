...
http://activemq.apache.org/[Apache ActiveMQ]是一个流行的开源消息传递提供商，很容易与Mule集成。 ActiveMQ支持JMS 1.1和J2EE 1.4规范，并在Apache 2.0许可下发布。

== 用法

要使用最常见的设置配置ActiveMQ连接器，请在Mule配置中使用`<jms:activemq-connector>`或`<jms:activemq-xa-connector>`（用于XA事务支持），例如：
...
[WARNING]
====
添加https://repository.apache.org/content/repositories/releases/org/apache/activemq/activemq-all/[ActiveMq-all.jar]会与其他依赖项产生冲突，所以请仅添加您需要的jar文件有关您需要ActiveMQ的一些信息：
...
使用failover：//协议连接到代理集群，并将其他ActiveMQ选项作为URI参数传递，例如：
