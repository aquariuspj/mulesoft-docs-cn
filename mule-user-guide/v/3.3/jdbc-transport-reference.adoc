...
某些功能仅适用于JDBC传输的Mule Enterprise版本。这些企业专用功能如下所述。
...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[SchemaDoc]  | image:check.png[（蜱）]  | image:check.png[（蜱）]  | image:check.png[（蜱）]  | image:check.png[（蜱）]（本地，XA）  | image:error.png[（错误）]  | image:check.png[（蜱）]  |单向请求回复 |单向 | org.mule.transport ：骡子运输-JDBC
|===
...
首先建立一个文件，它将保存下一批记录的起始点id。接下来你定义你的BatchManager并设置idStore，batchSize和起始点❷。然后你定义一个'noArgsWrapper'spring bean并设置批处理管理器的引用。 ❹是您定义在入站端点被触发后将被调用的组件的位置。您的出站端点可以使用
...
当您转到浏览器中的`http://localhost:8080/first20`时，以下内容将返回所有以xml格式处理的行：
...
请注意，存储过程仅在出站端点上受支持。如果要设置定期调用存储过程的流，可以定义一个 link:/mule-user-guide/v/3.3/quartz-transport-reference[石英]入站端点，然后在出站端点中定义存储过程调用。有关使用Quartz触发流的信息，请参阅以下 http://blog.mulesoft.org/2009/06/using-quartz-to-trigger-a-service/[博客帖子]。
...
事务在JDBC端点上受支持。有关详细信息，请参阅 link:/mule-user-guide/v/3.3/transaction-management[交易管理]。
...
这个例子需要Mule Enterprise运行。 ❶使用将在请求URL中传递的'max'参数定义选择数据库查询。我们在❷和define定义一些变换器将数据库行转换为xml，并为浏览器设置正确的Content-type以正确显示它。 ❹声明网址为`http://localhost:8080/rows`的http入站端点。由于我们在select查询中使用入站参数，因此我们还需要在请求的url上包含“max”参数，即http：// localhost：8080 / rows？max = 5。 ❺是jdbc出站端点在http端点被触发后将调用'selectRows'查询的位置。
...
您可以查看JDBC传输 http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[这里]的完整模式。架构文档的企业版本不可用。
...
*Tip* +
 +
使用`jdbc:derby-data-source`配置元素来配置Derby。如果您使用常规Bean，则在取消部署或重新部署应用程序时可能会收到错误。
...
|===
| {属性{1}}说明
|创建 |如果`true`数据库将在首次访问时创建。有关详细信息，请参阅 http://db.apache.org/derby/docs/10.7/ref/rrefattrib26867.html[德比文档]。
