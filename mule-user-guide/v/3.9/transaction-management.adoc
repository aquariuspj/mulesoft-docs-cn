...
*Note*：从Mule 3.5.0开始， link:/mule-user-guide/v/3.9/database-connector[数据库连接器]替换了JDBC连接器，并且不推荐使用JDBC连接器。使用JDBC传输的应用程序可以继续使用3.5.0 Enterprise Runtime，但该连接器在Studio调色板中不再可用于添加到应用程序。
...
|===
|类型 |特征 |资源数量 |可用资源 |性能
| link:/mule-user-guide/v/3.9/single-resource-transactions[单一资源交易] a |
* 只接收或发送消息到一个资源。
...
| link:/mule-user-guide/v/3.9/multiple-resource-transactions[多资源交易] a |
* 接收或发送消息给多个资源。
* 可以处理部分提交和回滚。
...
| link:/mule-user-guide/v/3.9/xa-transactions[XA交易] a |
* 接收或发送消息给多个资源。
...
虽然 link:/mule-user-guide/v/3.9/xa-transactions[XA交易]提供类似的功能，但 link:/mule-user-guide/v/3.9/multiple-resource-transactions[多资源交易]的开销较少。 XA交易更可靠。两者都可以包含JDBC资源。有关不同方法的讨论 - 包括多资源事务使用的1.5阶段提交概念 - 请参阅 link:http://www.javaworld.com/javaworld/jw-01-2009/jw-01-spring-transactions.html[JavaWorld关于分布式事务的文章]。
====

== 先决条件

本文档假定您熟悉Mule和/或 link:/anypoint-studio/v/6/[Anypoint Studio界面]。要提高您对Anypoint Studio的熟悉度，请考虑完成一个或多个 link:/getting-started/[Anypoint Studio快速入门]。此外，本文假设您具有交易处理的一些知识并且熟悉 link:/mule-user-guide/v/3.9/anypoint-connectors[Anypoint连接器]。
...
Use link:/mule-user-guide/v/3.9/transactions-configuration-reference[Transactions Configuration Reference] for quick access to attribute configurations.
....
...
_Apply a transaction to a flow that involves a single resource. See link:/mule-user-guide/v/3.9/single-resource-transactions[Single Resource Transaction] for details.
|XA Transaction |Apply a transaction to a flow that involves multiple resources: JMS, VM or JDBC. See link:/mule-user-guide/v/3.9/xa-transactions[XA Transaction] for details.
|Multi Transaction |Apply a transaction to a flow that involves multiple resources: JMS or VM. See link:/mule-user-guide/v/3.9/multiple-resource-transactions[Multiple Resource Transaction] for details.
...
* 如果将XA事务应用于流中的JMS入站连接器，则可以选择指定队列的轮询频率。有关配置详情，请访问 link:/mule-user-guide/v/3.9/xa-transactions[XA交易]。
...
请参阅 link:/mule-user-guide/v/3.9/error-handling[错误处理]文档以了解有关Mule的默认异常策略以及如何将异常策略应用于流程的更多信息。按照以下步骤将异常策略应用于单个事务。
...
+
. Configure the exception strategy as needed, keeping in mind Mule uses this exception strategy to handle any exceptions thrown while processing the transaction. Reference the link:/mule-user-guide/v/3.9/error-handling[Error Handling] documentation for exception strategy configuration details.
...
* 详细了解 link:/mule-user-guide/v/3.9/single-resource-transactions[单一资源交易]， link:/mule-user-guide/v/3.9/multiple-resource-transactions[多资源交易]和 link:/mule-user-guide/v/3.9/xa-transactions[XA交易]。
* 详细了解 link:/mule-user-guide/v/3.9/error-handling[例外策略]。
...
* 使用 link:/mule-user-guide/v/3.9/transactions-configuration-reference[事务配置参考]快速参考交易属性。
