...
*  Mule 3.8和更新版本接受SSH known_hosts文件。有关更多信息，请参阅 link:/mule-user-guide/v/3.9/sftp-transport-reference[SFTP传输参考]。
...
|===
| {字段{1}}说明
|  *Host Name*  | 代理主机名或Internet地址。
|  *Port Number*  | 代理端口号。
|  *User name*  |  _（可选）_代理身份验证的用户名。
|  *Password*  |  _（可选）_代理身份验证的密码。
|  *Protocol*  | 代理类型。允许的值是HTTP，SOCKS4和SOCKS5。
|===
...
有关使用XML编辑器设置SFTP端点属性的详细信息，请参阅 link:/mule-user-guide/v/3.9/sftp-transport-reference[SFTP传输参考]。
