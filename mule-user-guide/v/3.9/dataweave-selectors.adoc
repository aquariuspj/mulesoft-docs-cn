...
link:/mule-user-guide/v/3.9/mule-expression-language-mel[骡子表达语言（MEL）]表达。
...
您可以将Mule消息中的任何*Outbound Properties*到达DataWeave变换器并将其用于变换主体。要引用它，只需通过匹配的 link:/mule-user-guide/v/3.9/mule-expression-language-mel[骡子表达语言（MEL）]表达式来调用它。
...
您可以在DataWeave处理您的转换时引用服务器中存在的任何*Property*（系统或Spring），这样做可以使用*p('prop_name')*函数。
