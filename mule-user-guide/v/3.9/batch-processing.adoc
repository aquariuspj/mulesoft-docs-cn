...
*  link:/mule-user-guide/v/3.9/poll-reference[民意调查]。
...
当涉及批处理作业时，所有记录都将在 link:/mule-user-guide/v/3.9/batch-processing#process[处理阶段]开始之前首先存储在持久队列中，因此传统线程模型将不适用。
...
访问 link:/mule-user-guide/v/3.9/batch-processing-reference[批处理参考]了解更多信息。
====
...
* 每当Mule流执行批处理作业时，*batch job instance*就是Mule应用程序中的一个事件。 Mule在<<Load and Dispatch>>阶段创建批处理作业实例。每个_batch作业实例_都使用称为 link:/mule-user-guide/v/3.9/batch-job-instance-id[批量作业实例ID]的唯一字符串在内部标识。
====
...
由于Batch设计用于处理数百万条记录，因此它们需要存储在持久存储队列中。但是，考虑将一百万条记录放入队列中以执行三步批量作业。当Mule在 link:/mule-user-guide/v/3.9/batch-processing#batch-processing-phases[工作的阶段]中移动时，它会采取和请求每条记录，至少会发生三百万次I / O操作。
...
.  Mule通过 link:/mule-user-guide/v/3.9/splitter-flow-control-reference[收集分流器]发送消息有效载荷。这第一步触发创建一个新的批处理作业实例。
...
* 您可以在记录上设置*record variables*并将其从一步传递到另一个步骤（ link:/mule-user-guide/v/3.9/record-variable[阅读更多]）

* 您可以通过在每个批次步骤中添加*accept expressions*来应用过滤器，以防止该步骤处理某些记录;例如，您可以设置一个过滤器，以防止一个步骤处理上一步中处理失败的任何记录（ link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[阅读更多]）

* 您可以*commit*分组记录，将它们作为批量插入发送到外部来源或服务。 （ link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[阅读更多]）
...
如果将“完成”阶段保留为空（即，不在阶段中设置任何消息处理器），并且不在应用程序中的其他位置引用批处理作业结果对象，则批处理作业可以简单地完成，无论是失败还是成功。因此，良好做法规定您配置一些报告失败或成功记录的机制，以便在需要时采取进一步行动。有关批处理的可用MEL表达式列表，请参阅 link:/mule-user-guide/v/3.9/batch-processing-reference[批处理参考]。
...
使用放置在批处理作业输入阶段的*inbound, one-way message source*来触发批处理的开始。当它从外部源或服务接收数据时，消息源将启动批处理，从可能在输入阶段配置的任何准备开始。请参阅下面的示例，该示例利用 link:/mule-user-guide/v/3.9/poll-reference[轮询]功能定期从Salesforce获取数据。
...
--
+
使用 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[过滤器]指示后续批处理步骤如何处理失败的记录，而不考虑任何失败的记录.  *Continue processing*

.  *Continue processing*批处理，而不管任何失败的记录（使用 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[过滤器]指示后续批处理步骤如何处理失败的记录），直到批处理作业在该点累积*maximum number of failed records* Mule将所有记录推到“完成”阶段（理想情况下，您设计了一个报告来通知您失败的记录）
...
详细了解批量步骤中的 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[微调滤波器]，以更细化的级别管理失败的记录。


另请查看 link:https://docs.mulesoft.com/mule-user-guide/v/3.9/using-mel-with-batch-processing#mel-functions-that-involve-batch-jobs[批处理的MEL表达式]，了解可用于监控的MEL表达式的详细说明
...
如果批处理作业_没有_累积足够的失败记录以跨越`max-failed-records`阈值，则_all_记录 - 成功和失败 - 继续从批步骤到批步骤;使用 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[过滤器]来控制每个批次步骤处理的记录。
...
[TIP]
请注意，此示例介绍了本文档中未详细讨论的功能。请参阅 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[批量过滤器和批量提交]和 link:/mule-user-guide/v/3.9/record-variable[记录变量]了解更多信息。
...
. 应用程序首先使用 link:/mule-user-guide/v/3.9/file-connector[文件连接器]上传CSV文件，然后使用 link:/mule-user-guide/v/3.9/dataweave[DataWeave]将数据格式转换为集合（请参阅下面的映射）。集合中的每个项目都代表一个潜在客户。每个领导包含公司名称，名字，姓氏，生日和电子邮件地址。
+
...
[start=3]
.  Mule开始处理每个潜在客户作为记录。第一批步骤`lead-check`使用包裹有 link:/mule-user-guide/v/3.9/message-enricher[消息Enricher]的*Salesforce Connector*：
...
* 批处理不支持使用 link:/mule-user-guide/v/3.9/business-events[商业活动]。
...
* 在批处理中了解关于 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[过滤器]的更多信息。
* 详细了解 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[批量提交]。
* 详细了解如何设置和删除 link:/mule-user-guide/v/3.9/record-variable[记录级变量]。
* 详细了解您可以用来访问批处理数据的 link:/mule-user-guide/v/3.9/batch-processing-reference[MEL表达式]。
* 详细了解 link:/runtime-manager/cloudhub-fabric[运行时管理器支持批处理]。
* 详细了解 link:/mule-user-guide/v/3.9/anypoint-connectors[Anypoint连接器]。
* 详细了解 link:/mule-user-guide/v/3.9/poll-reference[轮询和水印]。
* 详细了解 link:/mule-user-guide/v/3.9/dataweave[DataWeave]。
