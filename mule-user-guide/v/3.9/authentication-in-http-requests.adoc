...
+
This MEL expression link:/mule-user-guide/v/3.9/authentication-in-http-requests#extracting-parameters-from-the-token-url-response[extracts an access token].
+
...
例如，重定向URL是 http://localhost:8082/callback in the link:/mule-user-guide/v/3.9/authentication-in-http-requests#creating-the-mule-client-app[前面的例子]：
...
当响应采用JSON格式时，会自动提取参数，并且您可以使用 link:/mule-user-guide/v/3.9/mule-expression-language-mel[MEL表达式]在Mule消息中引用这些值，这些值是对对令牌URL的请求的响应生成的，如前面的Github例。
...
您可以自定义Mule执行其中一个请求以使用 link:/mule-user-guide/v/3.9/mule-expression-language-mel[MEL表达]获取新的访问令牌。该表达式针对从HTTP请求连接器调用的响应生成的Mule消息进行评估。
...
默认情况下，令牌管理器使用内存中对象存储来存储凭据。您可以使用objectStore-ref属性自定义令牌管理器对象库。 link:/mule-user-guide/v/3.9/mule-object-stores[了解如何配置自定义对象库]。

== 另请参阅

*  HTTP请求连接器
*  HTTP侦听器连接器
* 查看此连接器中可用XML配置选项的 link:/mule-user-guide/v/3.9/http-connector-reference[参考]
