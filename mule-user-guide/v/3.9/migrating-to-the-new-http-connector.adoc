...
如果您要将项目从旧版本升级到Mule 3.9 Runtime，则应考虑进行的主要调整之一是替换旧版 link:/mule-user-guide/v/3.9/deprecated-http-transport-reference[基于HTTP端点的连接器]的任何实例以用于新的高性能 link:/mule-user-guide/v/3.9/http-connector[基于HTTP操作的连接器]。旧的基于HTTP端点的Connector仍然可以与Mule 3.6 Runtime一起工作，但它已被弃用，并且最终将在未来的版本中被删除。
...
新的HTTP连接器与旧的语法不同。除了不同的名称和不同的属性外，新连接器必须始终引用 link:/mule-user-guide/v/3.9/global-elements[全球元素]，这与Mule中大多数其他连接器的工作方式一致。多个连接器可引用封装了许多常用配置设置的单个全局元素。
...
在这两种情况下，Mule解析请求以生成一个更容易使用的Mule消息。 link:/mule-user-guide/v/3.9/http-listener-connector#the-request-body[阅读更多]。
...
在这两种情况下，Mule都会将相应的头文件添加到HTTP请求中并相应地构建消息体。 link:/mule-user-guide/v/3.9/http-listener-connector#the-request-body[阅读更多]。
