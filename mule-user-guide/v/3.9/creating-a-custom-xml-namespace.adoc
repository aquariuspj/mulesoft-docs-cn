...
每个模块都使用XML模式定义来定义模块可用于Mule Runtime的对象和属性。这些配置元素是使用每个模块的名称空间引入的，并将名称空间与模式关联起来。本页描述了如何在Mule中处理配置，以及在Mule中编写新模块或传输时需要执行哪些步骤。
...
注意：从Mule 3.0开始，版本不再是命名空间声明的一部分。另外，所有架构和XML配置文件中的"mulesource"都将替换为"mulesoft"。
...
其他名称空间可以通过`<spring:beans>`或通过在模块中添加专用元素来引入（请参阅 link:/mule-user-guide/v/3.9/scripting-module-reference[脚本模块的] `<lang>`元素）。
...
URI的第一部分 -  `http://www.mulesoft.org/schema/mule/`  - 对于每个模式都是相同的。然后是模块的简称，然后是`current`，Mule将其转换为正确的版本。
...
定义解析器是创建实际对象引用的地方。它包含一些Spring特定的类和术语，因此值得阅读 link:http://spring.io/blog/2006/08/28/creating-a-spring-2-0-namespace-use-spring-s-abstractbeandefintionparser-hierarchy[这个介绍]。请注意，Mule 3.7及更高版本使用Spring 4.1.6。
...
----
|从元素构造一个单独的独立bean。它不会被注入任何其他对象。这个解析器可以配置为自动设置对象的类，init和destroy方法，以及该对象是否为单例。
一个|
[source]
----
...
----
|此定义解析器将嵌套XML元素的分层处理概念引入。扩展此类的定义分析器始终是在父定义分析器上设置的子Bean。必须覆盖单个方法`getPropertyName`，以指定要使用此Bean在父Bean上设置的属性的名称。请注意，可以根据父元素动态解析属性名称。该实现还支持集合和地图。如果此元素的bean类设置为`MapEntryDefinitionParser.KeyValuePair`，则假定正在处理一个Map并将任何子元素添加到父Map中。
一个|
[source]
----
...
* 给出了一个有用的 link:_attachments/spring-forward-2006-about-jpa.ppt.pdf[PDF幻灯片]集，给出了一个介绍性例子，概述了Spring中的方法和（幻灯片29）。 Mule代码更复杂，但遵循相同的结构：`org.mule.config.spring.handlers.MuleNamespaceHandler`是命名空间处理程序; `org.mule.config.spring.parsers.AbstractMuleBeanDefinitionParser`和子类是bean定义解析器。

*  link:https://decaresystemsblog.wordpress.com/2006/04/04/spring-20-hiding-services-behind-custom-xml-schema-part-ii/[博客文章，给出了开发人员的视觉概述]。

* 关于可变和可扩展容器 link:http://www.xfront.com/VariableContentContainers.pdf[可变内容容器]和 link:http://www.xfront.com/ExtensibleContentModels.pdf[可扩展内容模型]的有用论文
