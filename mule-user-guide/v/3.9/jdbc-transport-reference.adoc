...
阅读有关数据库连接器功能的 link:/mule-user-guide/v/3.9/database-connector[*数据库连接器文档*]。
...
请注意，存储过程仅在出站端点上受支持。如果要设置定期调用存储过程的流，可以定义一个 link:/mule-user-guide/v/3.9/quartz-transport-reference[石英]入站端点，然后在出站端点中定义存储过程调用。
...
事务在JDBC端点上受支持。有关详细信息，请参阅 link:/mule-user-guide/v/3.9/transaction-management[交易管理]。
