...
注：直到成功组件需要强制 link:/mule-user-guide/v/3.9/mule-object-stores[对象存储]才能工作。然而，Anypoint Studio不强制执行此操作。如果没有声明对象存储，Studio将引发InitialisationException。有关更多信息，请参阅<<Until-Successful and Object Store>>。对象存储需要是每个直到成功实例都是独占的。
...
有关Mule中对象商店的更多信息，请参阅 link:/mule-user-guide/v/3.9/mule-object-stores[骡子对象商店]。以下代码示例说明如何配置内存存储：
...
* 详细了解Mule中的 link:/mule-user-guide/v/3.9/tuning-performance[调整性能]。
* 了解有关Mule中 link:/mule-user-guide/v/3.9/scopes[领域]的更多信息。
