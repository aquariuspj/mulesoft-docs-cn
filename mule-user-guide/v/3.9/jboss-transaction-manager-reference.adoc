...
该模块使Mule能够使用JBoss事务管理器（以前的Arjuna）来配置 link:/mule-user-guide/v/3.9/xa-transactions[XA交易]。开发人员可以配置每个Mule实例的一个Transaction Manger。有关详情，请参阅 link:http://www.jboss.org/jbosstm/[JBoss事务]， link:/mule-user-guide/v/3.9/deploying-mule-to-jboss[部署Mule到JBoss]， link:/mule-user-guide/v/3.9/transaction-management[交易管理]。
