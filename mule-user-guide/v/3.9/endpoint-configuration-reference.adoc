...
[NOTE]
====
本页讨论流级入站和出站端点的配置，这些端点是基于端点的Anypoint连接器的流级别元素。请注意，此页面上的信息均不适用于基于操作的连接器。有关一般连接器的更多信息，请参阅 link:/mule-user-guide/v/3.9/anypoint-connectors[Anypoint连接器]。
====
...
在最基本的形式中，端点充当 link:/mule-user-guide/v/3.9/connecting-using-transports[运输]及其基础数据源或消息通道的流级入站或出站实现。例如：
...
传统上，Mule中的端点被指定为 link:/mule-user-guide/v/3.9/mule-endpoint-uris[URI]，例如上面的示例。此表单仍然受支持，并且可能会根据您的应用程序而被证明更实用。但是，指定端点的推荐方法是通过特定于传输的名称空间，如以下示例所示。
...
出站端点也可以是动态的。这意味着端点的URI是 link:/mule-user-guide/v/3.9/mule-expression-language-mel[表达]的值，在发送消息之前对其进行评估。这允许消息的目标由其内容或任何消息属性的值确定。动态端点可以使用上述任何一种端点格式。
...
事件可以在通过端点接收或发送事件时开始或提交。端点必须是同步的，并且交易支持在很大程度上取决于所使用的特定传输。有关更多信息，请参阅 link:/mule-user-guide/v/3.9/transaction-management[交易管理]。
...
如果流程后面的某个内容抛出异常，则该文件将不会被使用，并且会被重新处理。 link:/mule-user-guide/v/3.9/idempotent-filter#idempotent-redelivery-policy[幂等-交还政策]确保它不会被重新处理超过3次;之后，它被发送到`vm:error-queue`，在那里它可以作为错误情况处理。
...
可以在端点上配置 link:/mule-user-guide/v/3.9/using-transformers[变形金刚]，在端点上封装转换逻辑，然后根据需要重新使用它。
...
以下参考表列出了可以为Mule中的 link:/mule-user-guide/v/3.9/generic-connector[通用终点]配置的属性。
