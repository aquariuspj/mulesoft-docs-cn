...
如果您想执行复杂的验证逻辑，或者重复使用已经包含在单独的JAR文件中的代码，那么 link:/mule-user-guide/v/3.9/validations-module#is-true-and-is-false-fallback-validators[`is-true`和`is-false`回退验证器]可能不是您最方便的解决方案。您可能会想要构建自己的验证器。
