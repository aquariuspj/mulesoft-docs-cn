...
选择流量控制使用 link:/mule-user-guide/v/3.9/mule-expression-language-mel[表达式]来评估消息的内容，然后将消息路由到其范围内的*routing options*之一（参见下图）。它将消息引导到与路由配置相匹配的范围中的第一个路由选项（计算结果为true）。如果没有任何表达式评估为true，则选择流控制会将消息引导至*default*（`else`）路由。
...
....
In Studio, drag the *Choice* icon from the Studio palette to the canvas, positioning it within the sequence of link:/mule-user-guide/v/3.9/elements-in-a-mule-flow[processors] that form the flow (below). 
...
* 有关Choice Flow Control的更多信息，请参阅Routing Message Processors页面上的 link:/mule-user-guide/v/3.9/routers[选择]部分。
