...
本文假定您有 link:/mule-user-guide/v/3.9/installing-anypoint-enterprise-security[安装了Anypoint Enterprise Security]个扩展。
...
*Encrypted Message Producer*应用程序由三个简单的 link:/mule-user-guide/v/3.9/mule-application-architecture[流动]组成，每个 link:/mule-user-guide/v/3.9/mule-application-architecture[流动]都充当Web服务提供者，它在将响应返回给调用者之前对消息进行加密或解密。根据功能组织，这些流程在应用程序中用于不同的目的。
...
确保右键点击任何 link:/mule-user-guide/v/3.9/mule-message-encryption-processor[Mule消息加密处理器]，然后选择`Add Encryption libraries to the project`。
...
* 有关Mule加密的更多信息，请参阅 link:/mule-user-guide/v/3.9/mule-message-encryption-processor[Mule消息加密处理器]。
* 有关Mule签名的更多信息，请参阅 link:/mule-user-guide/v/3.9/mule-digital-signature-processor[Mule数字签名处理器]。
*  https://docs.mulesoft.com/mule-user-guide/v/3.9/maven-tools-for-mule-esb [Maven Tools for Mule]
