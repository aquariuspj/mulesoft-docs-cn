=  Atom模块参考
:keywords: anypoint, atom, atom module, atompub, app
...
*Note*：在代码示例中，`spring-beans-current.xsd`是一个占位符。要找到正确的版本，请参阅链接：http：//www.springframework.org/schema/beans/ [Spring Beans版本。
...
|===
| {名称{1}}说明
| lastUpdate  |从中过滤事件的日期。在此日期之前上次更新的任何条目均不被接受。日期格式为日期格式为：yyyy-MM-dd hh：mm：ss，例如2016-12-25 13:00:00。如果只有日期很重要，您可以省略时间部分。您可以将该值设置为'now'以设置服务器启动的日期和时间。如果您想要接收所有可用条目，则不要设置此属性，然后再进行任何新条目。这是默认行为，适用于许多场景。
...
|===
| {名称{1}}说明
|路由 |为ATOM请求创建的URI请求路径。这匹配请求URL的路径。 route属性定义了一个基于Ruby on Rails风格路线的URI模板类型。例如："feed"或":feed/:entry"。作为参考，请参阅Ruby On Rails路由
...
|动词 |将由该过滤器接受的HTTP动词的逗号分隔列表。默认情况下，所有动词都被接受。
...
link:http://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/module/atom/package-summary.html[程序包摘要]
...
* 使用HTTP缓存。发送ETag和LastModified标题。识别304未修改的响应。这样你可以节省很多带宽。此外，某些脚本会识别LastModified标题并仅返回部分内容，例如仅返回两个或三个最新项目，而不是全部30个。
