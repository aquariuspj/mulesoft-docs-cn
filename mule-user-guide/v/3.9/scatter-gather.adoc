...
Scatter-Gather取代了在{Mule 3.5.0}中弃用的 link:/mule-user-guide/v/3.9/routers#All[所有]消息处理器。请注意，与All不同，Scatter-Gather并行地执行路由，而不是按顺序执行。并行执行路由可大大提高应用程序的效率，并提供比顺序处理更多的信息。有关您所期望的差异的更多信息，请参阅下面的<<Migrating to Scatter-Gather from the All Message Router>>。

== 先决条件

本文假定您熟悉Mule应用程序和 link:/mule-user-guide/v/3.9/message-processors[消息处理器]，特别是 link:/mule-user-guide/v/3.9/routers[路由消息处理器]。
...
|*Threading Profile* |Optionally, use to customize the threading profile. Refer to link:/mule-user-guide/v/3.9/tuning-performance[Tuning Performance] for a description of the configurable attributes. For a brief discussion of threading profiles in Scatter-Gather, see the section below. Default is `Default threading profile`. |
...
|===
|Optional Child Element |Description
|*threading-profile* |Allows you to configure the underlying thread pool. Refer to link:/mule-user-guide/v/3.9/tuning-performance[Tuning Performance] for a list of configurable attributes, all of which can be applied here. For a brief discussion of threading profiles in Scatter-Gather, see the section below.
|===
....
------
...
有关设置线程配置文件的详细信息，请参阅 link:/mule-user-guide/v/3.9/tuning-performance[调整性能]。

== 从全部消息路由器迁移到Scatter-Gather

如果您目前在您的应用程序中使用 link:/mule-user-guide/v/3.9/routers#All[所有]路由器，则可能希望将它们替换为Scatter-Gather路由器。本节详细介绍了在考虑迁移时需要注意的差异。

=== 为什么迁移？

对 link:/mule-user-guide/v/3.9/routers#All[所有]路由器的支持贯穿Mule 3.x系列。但是，MuleSoft建议迁移到Scatter-Gather，主要有两个原因：
...
link:/mule-user-guide/v/3.9/routers#All[所有]路由器实现顺序组播以通过指定的路由发送消息。这在一些情况下运行良好，如下所示：
