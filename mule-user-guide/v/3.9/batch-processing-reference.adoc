...
[NOTE]
 对于批处理作业在处理的每个阶段中采用的示例和步骤的*full description*，请参阅 link:/mule-user-guide/v/3.9/batch-processing[批量处理]。
...
* 详细了解 link:/mule-user-guide/v/3.9/batch-filters-and-batch-commit[批提交]。

* 详细了解如何设置和删除 link:/mule-user-guide/v/3.9/record-variable[记录级变量]。

* 了解 link:/mule-user-guide/v/3.9/using-mel-with-batch-processing[可以在批处理作业中使用的MEL表达式]以简化错误处理

* 查看Mule中批处理的 link:/mule-user-guide/v/3.9/batch-processing[基本的解剖学]。
