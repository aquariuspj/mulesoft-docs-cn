...
虽然很多时候您可以在不使用CXF的情况下代理Web服务（请参阅 link:/mule-user-guide/v/3.9/proxying-web-services[代理Web服务]），但有几种情况您可能需要使用CXF代理：
...
[TIP]
要将出站请求发送到外部Web服务，强烈建议您改用 link:/mule-user-guide/v/3.9/web-service-consumer[Web服务使用者]。
