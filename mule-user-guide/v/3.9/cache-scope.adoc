...
从Mule 3.5.0开始，默认情况下，新的缓存策略使用对象存储实例，该实例为所有群集节点提供统一的缓存而无需额外修改。例外是 link:/mule-user-guide/v/3.9/cache-scope[缓存范围]使用的默认缓存策略，该缓存策略使用InMemoryObjectStore，仅用于测试。
...
默认情况下，Mule将所有缓存的响应存储在InMemoryObjectStore中。 xref：ccs [创建一个缓存策略]并定义一个新的 link:/mule-user-guide/v/3.9/mule-object-stores[对象存储]，如果你想自定义Mule存储缓存响应的方式。
...
* 详细了解Mule中的 link:/mule-user-guide/v/3.9/scopes[领域]。
