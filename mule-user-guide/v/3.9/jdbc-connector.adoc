...
访问 link:/mule-user-guide/v/3.9/database-connector[数据库连接器]的文档以利用新设计的功能。
====
...
注：JDBC连接器支持多资源事务。 link:/mule-user-guide/v/3.9/database-connector[数据库连接器]也支持多资源交易。考虑使用 link:/mule-user-guide/v/3.9/xa-transactions[XA交易]来处理这个用例。
...
有关使用XML编辑器设置JDBC端点属性的详细信息，请参阅 link:/mule-user-guide/v/3.9/jdbc-transport-reference[JDBC传输参考]。
