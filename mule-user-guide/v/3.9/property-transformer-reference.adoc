...
使用*property transformer*在消息的出站范围上设置，删除或复制属性。该转换器仅转换_outbound_属性。请注意，您无法操作入站范围属性，因为这些属性是由消息源自动生成的。要详细了解消息范围，请参阅 link:/mule-user-guide/v/3.9/mule-concepts[骡子概念]。
...
在你设置一个新的属性后，你如何调用它并在流程中的其他地方使用它？

* 如果您在创建属性之前选择流中的任何组件，
您可以在*Outbound Properties*部分下的Metadata Explorer中看到该组件。
+
image:properties+metadata+explorer.jpeg[属性+元数据+资源管理器]
+
* 您可以在任何接受 link:/mule-user-guide/v/3.9/mule-expression-language-mel[骡子表达语言（MEL）]的组件的任何字段中引用该组件，并通过以下表达式调用该组件：
...
[TIP]
在Studio中，自动完成功能可以通过在该流的特定部分显示可用属性列表来帮助您。将光标置于一个字段中，然后按CTRL +空格查看字段选项。有关更多信息，请参阅 link:/anypoint-studio/v/6/#show-attribute-options[Anypoint Studio Essentials>显示属性选项]。

* 您可以在任何自定义Java类中引用组件，并通过以下方式调用它：
...
* 阅读相关转换器 link:/mule-user-guide/v/3.9/variable-transformer-reference[可变变压器]和 link:/mule-user-guide/v/3.9/session-variable-transformer-reference[会话变量变换器]，您可以使用它们为不同范围设置变量。
* 学习如何使用Mule表达式语言（MEL）来读取并在允许时使用 link:/mule-user-guide/v/3.9/mule-expression-language-mel[`inboundProperties`和`outboundProperties`映射]来操作属性。
