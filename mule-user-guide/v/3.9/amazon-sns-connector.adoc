...
适用于Amazon SNS的Anypoint连接器可提供与亚马逊简易通知服务（Amazon SNS）API的连接，使您能够构建分布式的支持Web的应用程序。 Mule应用程序可以使用Amazon SNS连接器轻松地将实时通知消息推送给感兴趣的用户，并通过多种传递协议。 MuleSoft在 link:/mule-user-guide/v/3.9/anypoint-connectors#connector-categories[_选择_]支持策略下维护此连接器。
...
您可以使用 link:/mule-user-guide/v/3.9/installing-connectors[安装连接器]中的说明在Anypoint Studio中安装连接器。
...
[NOTE]
请参阅最新版连接器https://mulesoft.github.io/mule-amazon-sns-connector/[here]的完整操作列表。
...
您可以从lhttps：//mulesoft.github.io/mule-amazon-sns-connector/ [github.io上的Mule Amazon SNS Connector]下载一个功能完整的示例。
