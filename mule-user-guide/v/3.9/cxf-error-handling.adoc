...
发布，使用或代理信息时，Mule中配置的 link:http://en.wikipedia.org/wiki/Apache_CXF[CXF Web服务]可能会遇到错误。 Mule为有效 link:/mule-user-guide/v/3.9/error-handling[处理错误]提供了一组*Exception Strategies*，当需要为涉及CXF Web服务的流自定义错误处理行为时，其中两个特别有用。
...
以下部分介绍如何使用Mule的 link:/mule-user-guide/v/3.9/error-handling[*默认异常策略*]， link:/mule-user-guide/v/3.9/rollback-exception-strategy[*回退异常策略*]或 link:/mule-user-guide/v/3.9/catch-exception-strategy[*捕捉异常策略*]处理CXF Web服务流中的错误。
...
请参阅 link:/mule-user-guide/v/3.9/rollback-exception-strategy[回滚异常策略]获取有关如何将一个流应用到流的详细说明。
...
请参阅 link:/mule-user-guide/v/3.9/catch-exception-strategy[捕捉异常策略]获取有关如何将一个流应用到流的详细说明。
