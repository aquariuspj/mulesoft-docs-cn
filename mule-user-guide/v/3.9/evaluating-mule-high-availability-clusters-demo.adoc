...
*Demo Starts Here*： link:/mule-user-guide/v/3.9/1-installing-the-demo-bundle[1  - 安装演示软件包]
...
*Learn more*： link:/mule-user-guide/v/3.9/mule-high-availability-ha-clusters[Mule高可用性（HA）群集概述]
...
.  *A demo application to deploy on the cluster.* +
 此*cluster-demo-app*由四个 link:/mule-user-guide/v/3.9/mule-application-architecture[流动]组成。应用程序接收来自JMS队列的消息，将消息添加到消息中，然后将它们分派给另一个JMS队列。它使用两种类型的事务来安全地处理消息：
