...
使用Mule link:/mule-user-guide/v/3.9/cxf-module-reference[CXF组件]，您还可以 link:/mule-user-guide/v/3.9/securing-a-soap-api[启用WS安全]，指定数据绑定，并将拦截器添加到您的CXF API。

向外部Web服务发送请求时，建议使用 link:/mule-user-guide/v/3.9/web-service-consumer[Web服务使用者]而不是CXF模块。
