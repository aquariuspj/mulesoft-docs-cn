...
如果您正在查找有关使用Transformer注释创建变压器的信息，请访问 link:/mule-user-guide/v/3.4/transformer-annotation[变压器注释]。
...
当Mule启动时，它会在加载任何配置之前发现该引导文件，并将该文件中列出的任何对象安装到本地注册表中。有关更多信息，请参阅 link:/mule-user-guide/v/3.4/bootstrapping-the-registry[引导注册表]。
