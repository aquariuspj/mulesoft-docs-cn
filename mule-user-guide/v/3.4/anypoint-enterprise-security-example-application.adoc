...
作为一个简单的演示，本示例应用程序提供了有关如何使用两个Anypoint Enterprise Security功能的见解：Mule数字签名处理器和Mule消息加密处理器。演示说明了如何使用Mule完成以下任务：
...
本文档假定您熟悉 link:/anypoint-studio/v/5/[Mule Studio Essentials]指南。要提高您对Studio的熟悉度，请考虑完成 link:/anypoint-studio/v/5/basic-studio-tutorial[基本工作室教程]。
...
. 使用*Zip File*字段旁边的epllipsis按钮（...）浏览并选择您下载的Mule应用程序：`anypoint_enterprise_security_example.zip`。
. 点击*Finish*。 Studio导入工具随即打开该项目。

== 设置，然后运行应用程序

. 在*anypoint_enterprise_security_example*项目中，将Studio画布向下滚动到*Get_secret_file*流程。
. 双击*Groovy*组件以打开其“模式属性”面板。
...
*Encrypted Message Producer*应用程序由三个简单的[流程]组成，每个流程都充当一个Web服务提供者，它在向调用者返回响应之前加密和解密消息。根据功能组织，这些流程在应用程序中用于不同的目的。

*  *Get_CC_information*接收客户信用卡信息的Web服务请求，然后将其检索，加密，然后返回给调用者（参见下面的图片顶部）。
*  *Decrypt_The_Request*解密接收到的消息记录解密活动，然后将未加密的消息内容（本例中为信用卡信息）返回给调用者。 （见下图，中间）。
...
|操作 |指示元素验证签名，而不是应用签名。
|输入参考 |使用Mule表达式来定义数字签名所适用的有效负载部分。
...
最终用户向Mule应用程序提交请求以解密消息有效负载，以获取客户未加密的信用卡信息。使用HTTP请求工具（例如Chrome网络浏览器中的https://chrome.google.com/webstore/detail/rest-console/cokgbflfommojglbmbpenpphppikmonn [RES控制台插件]）将以下请求发布到`+http://localhost:8081/decrypt+`
...
消息加密处理器引用*plainXML* globabl加密元素以根据以下配置进行操作：
...
最后，Mule使用HTTP端点将响应 - 解密的信用卡信息传递给最终用户的浏览器。下面的代码显示未加密的信用卡信息。
...
最后，使用Mule使用HTTP端点将响应（`encrypted.txt`文件中的解密消息）传递给最终用户的浏览器（参见下图）。

image:get_secret_file_response.png[get_secret_file_response]

== 另请参阅

* 有关Mule加密的更多信息，请参阅 link:/mule-user-guide/v/3.4/mule-message-encryption-processor[Mule消息加密处理器]。
* 有关Mule签名的更多信息，请参阅 link:/mule-user-guide/v/3.4/mule-digital-signature-processor[Mule数字签名处理器]。
