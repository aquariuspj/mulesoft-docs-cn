...
本页介绍如何将Mule与Open MQ JMS代理（社区版本 http://docs.sun.com/app/docs/coll/1307.3[Sun Java System Message Queue]）集成。它包括配置直接连接和JNDI连接的示例。
...
. 使用来自Sun http://docs.sun.com/app/docs/doc/819-7755/6n9m8u57v?a=view#aeoay[这里]的指示配置Open MQ。您必须使用Open MQ或Message Queue管理控制台来定义connectionFactory（工厂类型主题/队列连接工厂）并配置您将在Mule配置文件中指定的任何Open MQ JMS端点。
