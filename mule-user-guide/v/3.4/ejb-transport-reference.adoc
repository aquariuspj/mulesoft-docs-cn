...
EJB传输允许将EJB会话bean作为事件流的一部分进行调用。组件可以被赋予一个EJB出站端点，该端点调用远程对象并可选地返回结果。
...
EJB端点的配置方式与 link:/mule-user-guide/v/3.4/rmi-transport-reference[RMI端点]相同。

*Note*：只有出站端点才能使用EJB传输。
...
如果该方法可以接受一个或多个输入参数，请使用`methodArgumentTypes`属性将它们的类型配置为逗号分隔列表。多个参数作为Mule消息的有效载荷以对象数组的形式传入。
