...
Mule支持三种不同类型的事务：单一资源，多种资源和扩展架构（XA）。下表介绍了每种类型的一些特性。


[%header%autowidth.spread]
|===
|类型 |特征 |资源数量 |可用资源 |性能
| link:/mule-user-guide/v/3.4/single-resource-transactions[单一资源交易] a |
* 只接收或发送消息到一个资源。
| 1 a |
*  JMS
*  VM
...
一个|
* 与其他人相比，表现更好
| link:/mule-user-guide/v/3.4/multiple-resource-transactions[多资源交易] a |
* 接收或发送消息给多个资源。
* 可以处理部分提交和回滚。
...
* 比XA执行得更好，但比单一资源慢
带有1.5PC的* ，功能不如XA。
| link:/mule-user-guide/v/3.4/xa-transactions[XA交易] a |
* 接收或发送消息给多个资源。
...
尽管XA事务处理提供了类似的功能，但多资源事务使用的开销较小另一方面，XA事务更可靠，可以包含JDBC资源。有关不同方法的讨论 - 包括多资源事务使用的1.5阶段提交概念 - 请参阅 http://www.javaworld.com/javaworld/jw-01-2009/jw-01-spring-transactions.html[JavaWorld关于分布式事务的文章]。
====

== 配置事务

您可以通过将事务配置应用于端点或通过将多个元素包装在事务包装中来划分事务。

*  *Apply a transaction to an inbound endpoint*当您希望Mule将整个流程作为一个事务处理时。
*  *Apply a transaction to an outbound endpoint*当您希望Mule将传出操作作为现有事务的一部分处理时。
当您想要将事务应用于流中的_do流中的元素时，{_0}} *Apply a transaction as a wrapper*（在Studio中称为*scope*），该流中的_do not_开始于配置为事务的入站端点。
...
* 如果您将XA事务应用于流程中的多个端点，请确保将连接器配置为使用**XA-enabled resources**。
* 如果您将XA事务应用于流中的JMS入站端点，则可以选择指定队列的轮询频率。有关配置详情，请访问 link:/mule-user-guide/v/3.4/xa-transactions[XA交易]。
*  Mule可以管理** non-transactional outbound endpoints.**默认情况下，来自非事务性传输的出站端点忽略活动事务而不是拒绝它。换句话说，这些端点的默认事务操作不再是`NONE`。下面的示例代码演示了这种行为。 Mule会同步处理从VM队列接收到的消息。代码示例中的文件传输不是事务性的，因此写入文件不是事务的一部分。但是，如果消息在Mule创建文件时抛出异常，Mule将回滚事务并重新处理消息。这个例子实际上是一个多资源事务。
...
. 为流或事务配置没有例外策略，因此默认采用Mule的默认异常策略。
...
请参阅 link:/mule-user-guide/v/3.4/error-handling[错误处理]文档以了解有关Mule的默认异常策略以及如何将异常策略应用于流程的更多信息。按照以下步骤将异常策略应用于单个事务。
...
* 详细了解 link:/mule-user-guide/v/3.4/single-resource-transactions[单一资源交易]，多资源交易和 link:/mule-user-guide/v/3.4/xa-transactions[XA交易]。
* 详细了解 link:/mule-user-guide/v/3.4/error-handling[例外策略]。
...
* 使用 link:/mule-user-guide/v/3.4/transactions-configuration-reference[事务配置参考]快速参考交易属性。
