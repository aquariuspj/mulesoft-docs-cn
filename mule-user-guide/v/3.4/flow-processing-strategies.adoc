...
* 必须通知流程入站端点处理消息期间发生的所有错误。这种情况在 link:/mule-user-guide/v/3.4/reliability-patterns[可靠性模式]中进一步讨论。
...
* 在Mule为流选择排队异步处理的情况下，您可以指定同步流（请参阅<<Forcing a Specific Flow Processing Strategy>>）。您可能需要指定一个同步流程来实现可靠性。回想一下，同步策略非常适用于流程入站端点必须被告知处理消息期间发生的所有错误的流程。这种情况在 link:/mule-user-guide/v/3.4/reliability-patterns[可靠性模式]中进一步讨论。 +
 请注意，您无法强制同步流成为异步。
