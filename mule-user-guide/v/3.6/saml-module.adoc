...
Mule的 link:/mule-user-guide/v/3.6/cxf-module-reference[CXF模块]包括带有支持SAML 2.0的SAML选项的WS-Security。
====
...
由于SAML用于单点登录，因此假定用户的身份验证已经发生，并且SAML令牌只包含一个或多个*subjects*，其中提供了一些其他系统可以理解的信息。在这种情况下，我们将配置我们的流程以要求`AllowGreetingServices`的SAML主题。对于我们的入站连接器，我们添加一个带有回调的`SAMLVerifyInterceptor`，它将检查正确的SAML主题：
...
|===
| {名称{1}}基数 |说明
| saml-security-provider  | 0..1  |将授权委托给其他提供者的安全提供者。
|===
