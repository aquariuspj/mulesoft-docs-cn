...
*  link:/mule-user-guide/v/3.6/setting-up-ldap-provider-for-spring-security[LDAP]与第三方身份管理系统集成
*  link:/mule-user-guide/v/3.6/enabling-ws-security[入站请求的验证]通过SAML 2.0联合身份标准
*  link:/mule-user-guide/v/3.6/sftp-transport-reference[安全FTP（SFTP）传输]使Mule流能够通过SSH协议读取和写入远程目录。
...
link:/mule-user-guide/v/3.6/mule-digital-signature-processor[在Mule的消息中添加数字签名] +
link:/mule-user-guide/v/3.6/mule-digital-signature-processor[签署消息有效负载的一部分] +
link:/mule-user-guide/v/3.6/mule-digital-signature-processor[验证Mule中的数字签名]

====  *Mule Filter Processor*

Mule可以过滤收到的信息，避免处理无效信息。使用过滤器处理器后，Mule会丢弃它收到的任何与过滤器参数不匹配的消息 - 例如，来自设置的IP地址范围之外的消息。

link:/mule-user-guide/v/3.6/anypoint-filter-processor[过滤不安全的消息]
