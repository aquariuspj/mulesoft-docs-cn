...
您可以使用 link:/mule-user-guide/v/3.7/installing-connectors[从Anypoint Exchange安装连接器]中的说明在Anypoint Studio中安装连接器。
...
请联系mailto：info@mulesoft.com [MuleSoft销售团队]安排授权。另请参阅 link:/mule-user-guide/v/3.6/installing-an-enterprise-license[安装企业许可证]。
...
. 为您的新项目输入一个名称，并将其余选项保留为默认值。 +
  image:New+Project-1.png[新建项目+ 1]
...
[NOTE]
====
如果您打算将Siebel连接器与 link:/anypoint-studio/v/6/datamapper-user-guide-and-reference[DataMapper转换器]结合使用来映射和转换数据，则可以使用Anypoint Studio的 link:/mule-user-guide/v/3.6/datasense[DataSense]功能。您可以启用DataSense作为配置全局元素的一部分。

* 按照DataSense指示信息将转换器放入流中，然后使用Siebel连接器将 link:/mule-user-guide/v/3.6/testing-connections[测试连接]转换为Siebel。
...
| *General:*  |
“常规”部分中的值因您选择的操作而异。以下字段特定于查询业务组件操作。有关所有可用选项的详细说明，请访问Siebel连接器的Mule API参考文档。 |
...
| *Search Expression*  |使用 link:/mule-user-guide/v/3.6/mule-expression-language-mel[骡子表达语言（MEL）]定义将返回业务组件列表的搜索表达式。 |
| *View Mode*  |为连接器返回的结果定义Siebel视图模式。 | `3`
| *Fields to Retrieve*  |使用此部分指定要在查询中检索的字段列表：* +
来自消息：*指定要在传入有效负载中检索的业务组件字段。 +
  **Create Object manually: **使用Object Builder编辑器手动指定字段。 | `From Message`
| *Search Spec*  |指定要在搜索查询**中用作过滤器的搜索值：+
来自消息：**定义在传入有效内容中将哪些值用作搜索过滤器。 +
...
|===
|字段 |描述 |默认值
| *Display Name*  |为应用程序中的连接器定义唯一的标签。 | Oracle Siebel eBusiness Services
| *Connector Configuration*  |选择您创建的全局Siebel连接器元素。 |
| *Operation*  |定义此组件必须执行的操作：定义此组件必须执行的操作：
*Execute:*使用SiebelPropertySets执行Siebel服务。 *Execute business service:*使用地图代替SiebelPropertySet执行Siebel服务。 |
3 + | *General*
3 + |如果您选择*Execute*操作：
| *Integration Object*  |定义要对其执行操作的Siebel集成对象类型。 |
| *Method Name*  |输入要执行的方法的名称。 |
| *Service Name*  |输入要执行的Siebel服务的名称 |
| *Input Properties*  | *From Message*：在传入负载中定义SiebelPropertySet。 *Create Object manually*：手动定义SiebelPropertySet  |
3 + |如果您选择*Execute business service*操作：
| *Business Service*  |输入要执行的Siebel服务的名称。 |
| *Input*  | *From Message:*指定要在传入有效内容中执行的服务。 +
*Create Object manually:*指定要手动执行的服务。 |
|===

. 单击画布上的空白区域以保存您的Oracle Siebel Business Services连接器配置。
...
|===
|字段 |描述 |默认值
| *Display Name*  |为应用程序中的连接器定义唯一的标签。 | `Oracle Siebel Integration Objects `
| *Connector Configuration*  |选择您创建的全局Siebel连接器元素。 |
| *Operation*  |定义此组件必须执行的操作：执行Siebel Adapter。 | `Execute Siebel Adapter  `
3 + | *General:*
| *Integration Object*  |定义要对其执行操作的Siebel集成对象类型。 |
| *Method*  |定义EAI Siebel适配器方法。 |
| *Input Properties*  | *From Message:*映射传入有效负载中的集成对象字段。 +
  *Create Object manually:*使用Object Builder编辑器手动映射Integration Object字段。 | `From Message`
|===
...
[NOTE]
有关这些Mule元素的详细信息，请参阅 link:/mule-user-guide/v/3.6/poll-reference[投票范围]， link:/anypoint-studio/v/6/datamapper-user-guide-and-reference[的DataMapper]和 link:/mule-user-guide/v/3.6/salesforce-connector[Salesforce连接器]中的文档。
...
[NOTE]
有关这些元素的深入信息，请参阅 link:/mule-user-guide/v/3.6/poll-reference[投票范围]， link:/anypoint-studio/v/6/datamapper-user-guide-and-reference[的DataMapper]和 link:/mule-user-guide/v/3.6/salesforce-connector[Salesforce连接器]中的文档。
...
* 了解 link:/anypoint-studio/v/6/datamapper-user-guide-and-reference[的DataMapper]转换器，这是将映射分配给数据的最有效方式。
