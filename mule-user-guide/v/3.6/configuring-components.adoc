...
组件包含用于处理通过Mule ESB传递的消息的业务逻辑。组件可以是任何类型的对象，包括 link:/mule-user-guide/v/3.6/using-spring-beans-as-flow-components[春豆]， link:/mule-user-guide/v/3.6/configuring-java-components[POJO]， link:/mule-user-guide/v/3.6/scripting-module-reference[脚本]， link:/mule-user-guide/v/3.6/using-web-services[网络服务]或REST调用。

由于它们对于您的实施非常具体，因此您通常会 link:/mule-user-guide/v/3.6/developing-components[创建您自己的自定义组件]，或者只是使用现有的POJO。 Mule还附带一些标准组件，您可以根据需要使用或扩展它们。本页介绍如何配置不同类型的组件。
...
| *<test:component/>*  |配置Mule `FunctionalTestComponent`，它允许创建更复杂的测试场景。有关更多信息，请参阅 link:/mule-user-guide/v/3.6/functional-testing[功能测试]。
|===
...
如果向组件添加拦截器，则需要显式语法而不是快捷方式`<component class>`语法。有关应用的默认设置的更多配置选项和信息，请参阅 link:/mule-user-guide/v/3.6/configuring-java-components[配置Java组件]。
