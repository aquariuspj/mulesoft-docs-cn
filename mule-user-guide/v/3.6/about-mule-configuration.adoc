...
*  link:/mule-user-guide/v/3.6/custom-message-processors[自定义消息处理器]  - 观察消息，或修改消息或消息流。例子包括变压器和滤波器。
...
*  link:/mule-user-guide/v/3.6/configuring-a-transport[连接器]  - 使用的任何传输的非默认配置
...
*  link:/mule-user-guide/v/3.6/configuring-security[安全经理]  - 基于一个或多个安全提供程序验证请求

*  link:/mule-user-guide/v/3.6/mule-agents[代理]  - 代理通常用于横切关注点，例如日志记录或管理

*  link:/mule-user-guide/v/3.6/mule-server-notifications[通知]  - 允许您在某些生命周期事件中收到通知

*  link:/mule-user-guide/v/3.6/transaction-management[交易管理]  - 在入站端点上配置Mule事务，其中端点可配置为启动新事务或加入现有事务。

*  link:/mule-user-guide/v/3.6/configuring-properties[属性]  - 属性占位符，消息属性和系统属性。
...
有关可用全局配置设置的列表，请参阅 link:/mule-user-guide/v/3.6/global-settings-configuration-reference[全局设置配置参考]。
