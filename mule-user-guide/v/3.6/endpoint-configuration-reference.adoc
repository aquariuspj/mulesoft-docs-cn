...
[NOTE]
====
本页讨论流级入站和出站端点的配置，这些端点是基于端点的Anypoint连接器的流级别元素。请注意，此页面上的信息均不适用于基于操作的连接器。有关一般连接器的更多信息，请参阅 link:/mule-user-guide/v/3.6/anypoint-connectors[Anypoint连接器]。
====
...
在最基本的形式中，端点充当 link:/mule-user-guide/v/3.6/connecting-using-transports[运输]及其基础数据源或消息通道的流级入站或出站实现。例如：
...
出站端点也可以是动态的。这意味着端点的URI是 link:/mule-user-guide/v/3.6/mule-expression-language-mel[表达]的值，在发送消息之前对其进行评估。这允许消息的目标由其内容或任何消息属性的值确定。动态端点可以使用上述任何一种端点格式。
...
以下参考表列出了可以为Mule中的 link:/mule-user-guide/v/3.6/generic-connector[通用终点]配置的属性。
