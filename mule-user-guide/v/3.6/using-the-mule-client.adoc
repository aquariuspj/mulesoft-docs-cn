...
为了处理发生多个结果的情况，Mule引入了一个新的消息类型 link:http://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection]。

* 这种类型的消息按收到的顺序包含所有消息结果。请注意， link:http://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection]扩展了 link:http://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/api/MuleMessage.html[org.mule.api.MuleMessage]，因此界面相似。如果有多个结果，则`MuleMessage.getPayload()`方法返回包含每个返回消息的有效内容的`java.util.List`。
