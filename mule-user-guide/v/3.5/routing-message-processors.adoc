...
消息处理程序在流程中用于控制消息在该流程中的发送和接收方式。这在 link:/mule-user-guide/v/3.5/using-flows-for-service-orchestration[使用流进行服务编排]中有进一步描述。
...
| link:/mule-user-guide/v/3.5/until-successful-scope[直到成功]  |重复尝试处理消息直至成功
| <<Splitter>>  |使用表达式分割消息
...
*From 3.1.0*您可以通过指定_'failureExpression'_来进一步自定义此路由器的行为，您可以使用 link:/mule-user-guide/v/3.5/non-mel-expressions-configuration-reference[Mule表达式]来定义故障。 _failureExpression_属性配置如下：
