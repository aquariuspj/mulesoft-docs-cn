...
异步范围与 link:/mule-user-guide/v/3.7/flows-and-subflows[异步流程]类似：
...
异步范围类似于 link:/mule-user-guide/v/3.7/flows-and-subflows[子流]，因为它继承了主流的异常策略。
...
[IMPORTANT]
即使异步作用域收到Mule消息的副本，也不会复制有效内容。这两个Mule消息引用了相同的有效负载对象：一个继续沿着原始流，另一个由Async范围处理。
...
[source, xml, linenums]
----
<mule xmlns:http="http://www.mulesoft.org/schema/mule/http" xmlns:jms="http://www.mulesoft.org/schema/mule/jms" xmlns:spring="http://www.springframework.org/schema/beans" version="EE-3.5.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.springframework.org/schema/beans http://www.springframework.org/schema/beans/spring-beans-current.xsd
 
