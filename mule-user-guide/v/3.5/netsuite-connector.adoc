...
用于NetSuite的Anypoint Connector可同步数据并自动化NetSuite和第三方应用程序之间的业务流程，无论是内部部署还是云中。

== 假设

本文档假设您熟悉Mule，[Anypoin（TM）连接器]和[Anypoint Studio界面]。为了增加对Studio的熟悉度，请考虑完成一个或多个[Anypoint Studio教程]。此外，本页假定您对[Mule流]和[Mule Global Elements]有基本的了解。

本文档描述了Anypoint Studio，Mule ESB的图形用户界面环境中的实现示例，还包括在XML编辑器中执行相同的配置细节。

== 在Anypoint Studio中安装NetSuite连接器

您可以通过在Anypoint Studio中安装NetSuite连接器"test drive"。按照说明[下载并启动Anypoint Studio]，然后按照以下步骤安装NetSuite连接器。

要在Anypoint Studio中安装NetSuite连接器，请执行以下操作：

. 在Anypoint Studio的*Help*菜单下，选择*Install New Software*。
+
image:Install.jpeg[安装]

. 在*Install*向导上，单击*Work with:*字段的下拉箭头，选择Anypoint Connector更新站点
+
image:workwith.jpeg[workwith]

. 在*Filter*字段下方的表格中，单击以展开*Standard*文件夹，然后单击*NetSuite Connector (Mule 3.5+). Click *下一个*的复选框。
+
image:SelectNetSuite.jpeg[SelectNetSuite]

. 在安装详细信息中，单击*Next*。

. 在审核许可中，查看许可文本，然后点击*I accept the terms of the license agreement*。点击*Finish*。连接器安装。

. 安装结束后，请点击*Yes*重新启动Anypoint Studio。 Anypoint Studio重新启动后，NetSuite连接器将显示在连接器列表中。
+
...
* 全局NetSuite元素，供应用程序中的所有NetSuite连接器使用。有关更多信息，请参见[Mule全局元素]。
