...
`log4j.properties`文件必须位于类路径的根目录下。如果您没有`log4j.properties`文件，则可以获得简单的 link:https://github.com/mulesoft/mule/blob/mule-3.5.x/distributions/standalone/src/main/resources/conf/log4j.properties[这里]。有关配置Log4J的更多信息，请参阅其 http://logging.apache.org/log4j/1.2/[网站]。
