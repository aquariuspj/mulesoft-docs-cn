...
您可以配置Tcat控制台以利用LDAP服务器进行用户验证。在此用例中，LDAP服务器创建并管理用户，并且控制台上不存储有关用户的信息。组信息在LDAP服务器和控制台上进行管理。
...
例如，您可以在名为`tcat`的LDAP数据库上创建用户，并将控制台配置为使用Tcat的凭据连接到LDAP服务器。例如，当用户以用户`admin`身份登录控制台时，控制台首先使用Tcat的凭据对LDAP服务器上的_itself_进行身份验证，然后将管理员凭据发送到LDAP服务器。如果管理员的凭证有效，则LDAP服务器会向控制台提供管理员的组信息。然后，控制台根据为`admin`所属的组指定的权限向`admin`授予适当的权限。
...
*  *The LDAP user account credentials that the console uses to connect to LDAP*。这是在LDAP服务器上配置的用户帐户，它允许控制台访问存储有关控制台用户信息的数据库。示例：用户：`tcat`密码：`tcat123`
...
*  `<Tcat install path>/conf/Catalina/localhost/tcat-env.conf`，它指示控制台启用LDAP验证所需的LDAP弹簧配置文件。
*  `<Tcat install path>/webapps/console/WEB-INF/classes/META-INF/ldap.properties`，其中包含连接到LDAP服务器以及搜索该服务器上的用户所需的信息。

=== 启用LDAP Spring Profile

完成以下步骤：

. 删除文件`<Tcat install path>/bin/setenv.sh (or setenv.bat)`
. 转到以下目录：`<Tcat install path>/conf/Catalina/localhost/ `
. 找到文件`tcat-env.conf`。
...
. 转到`classpath`目录：`<Tcat install path>/webapps/console/WEB-INF/classes/META-INF`
. 创建/编辑名为`ldap.properties`的文件。这是LDAP配置文件。
...
控制台会自动为属于组`Administrators`的任何用户分配完全管理权限，因此在登录时，您拥有控制台上的全部管理权限。然后，您可以在控制台上创建其他组并分配所需的权限。您可以在LDAP服务器上执行所有其他用户配置，例如从组中添加/删除用户。之后，您可以根据需要从LDAP服务器中删除`Administrators`组。
...
* 操作系统：Linux（Xubuntu 12.04 LTS，基于Debian 7“Wheezy / Sid”）。主页：http://xubuntu.org
...
此外，还有几个第三方预打包版本可供下载。其中，Symas Corporation包含Microsoft Windows的付费版本。
...
==== 在某些基于Debian的系统上以root身份运行命令

[WARNING]
====
在一些基于Debian的系统上，例如Ubuntu或其衍生产品，通常以root身份运行命令的过程是发出：`sudo <command>`

或者，您可以通过运行（作为在`/etc/sudoers`文件中正确授权的用户）以下命令来成为root用户：
...
安装Apache Directory Studio非常简单 - 只需解压并解压安装文件并运行程序二进制文件。可以找到每个支持的平台的安装说明 http://directory.apache.org/studio/users-guide/apache_directory_studio/download_install.html[这里]。
...
在本例中，我们修改`slapd`的配置以从系统范围的LDAP配置文件`/etc/ldap/ldap.conf`中读取其参数。然后，我们修改此文件以包含包含LDAP用户的数据库的定义。这包括这些步骤：
...
----
+
这告诉`slapd`守护进程从系统范围的LDAP配置文件`/etc/ldap/ldap.conf`中读取其配置，我们在下一步中修改该配置文件`/etc/ldap/ldap.conf`。
. 找到以`SLAPD_PIDFILE`开头的行，并重新写入以读取：
+
[source, code]
----
...
. 找到以`SLAPD_SERVICES`开头的行，并重新写入以读取：
+
[source, code]
----
...
[NOTE]
====
如<<Modify the System-Wide LDAP Configuration File>>所示，LDAP配置文件存储数据库根用户的用户名和密码。密码可以以明文保存;然而，这构成安全风险，因为LDAP配置文件是世界可读的。要为密码生成散列，请运行以下命令：`slappasswd -h <scheme> -s <secret>`
...
|===
|用户 |此配置中的用户功能
| tcat  |允许控制台对LDAP上的_itself_进行身份验证，以获得对数据库的读取访问权限。该用户的凭证存储在控制台的LDAP配置文件`ldap.properties`中。
| admin  |这是一个控制台用户。它是`Administrators`组的成员，并且在控制台上具有完整的管理权限。
...
. 在*=*字段中输入`tcat`。当您这样做时，*DN Preview*窗口应该自动填入您创建的条目的*Distinguished Name*，在这种情况下`cn=tcat,ou=people,dc=company,dc=com`。
. 按*Next*。
. 当*Attributes*窗口提示您为*sn*字段指定值时，右键单击*sn*字段，然后选择*Edit Value*。键入*tcat*，然后点击*Finish*。
...
+
. 提示输入新密码时，请在*Enter New Password*字段中输入*tcat123*（或您选择的另一个密码;在控制台的`ldap.properties`文件中指定相同的密码）。
. 在*Select Hash Method*菜单中，选择*Plaintext*，然后点击*OK*。

用户`tcat`现在已使用此示例配置中使用的所有属性进行了定义。
...
当用户登录到控制台时，控制台首先要求LDAP服务器验证用户名和密码，然后请求该用户所属的组的列表。此搜索的参数在`ldap.properties`文件的以下行中定义：
...
按照<<Enabling LDAP on the Console>>中概述的步骤操作。在本例中，`ldap.properties`文件的内容如下所示：
...
登录到控制台后，验证控制台是否已正确读取LDAP服务器中的所有用户。为此，请点击*Administration*。您应该看到在LDAP数据库上创建的所有用户的列表，在本例中为tcat `admin`和`testuser1`。
...
从控制台注销，然后以*testuser1*身份登录。控制台显示仪表板，但限制了与组`testgroup`的有限权限相匹配的选项。
...
以下是本示例中创建的所有数据库用户和组对象的LDIF文件。如果您希望自动将此用户和组配置复制到LDAP数据库中，请将以下代码复制粘贴到纯文本文件中，然后使用以下命令导入文件：
