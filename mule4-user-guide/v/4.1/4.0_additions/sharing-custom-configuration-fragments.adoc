...
如果此flow-ref在同一应用程序中，则不需要进一步的配置即可调用子流。但是，如果此flow-ref与其调用的子流不同，则需要先将包含子流的共享配置文件打包到JAR文件中。然后，使用 link:/mule-user-guide/v/3.9/sharing-custom-code[共享自定义代码]中描述的Maven依赖关系机制将其包含在您的应用程序中。最后，导入配置文件如下。
