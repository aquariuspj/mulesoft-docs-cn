...
要使用Web服务使用者配置MTOM客户端，您必须指定`mtomEnabled`属性。消息有效负载应包含一个保存与消息一起传输的附件（如文本，图像或应用程序）的子部分。有关附件的更多信息，请参阅 link:/mule-user-guide/v/3.9/attachment-transformer-reference[附件变压器参考]。
