...
Mule的能力之一是处理 link:/mule-user-guide/v/3.7/batch-processing[批次]中的消息。此功能由`<batch:commit>`块提供，特别适用于需要收集批量upsert到外部源或服务的记录子集时。但是，除非所述连接器支持批量操作，否则不能在批处理块内使用。
...
一些已有的连接器（例如 link:/mule-user-guide/v/3.7/salesforce-connector[销售队伍]和 link:http://www.mulesoft.com/cloud-connectors/netsuite-integration-connector[NetSuite的]）已经支持批量操作多年。但是，通过引入新的批处理模块，开发人员不再需要解析连接器的响应，以确定哪些记录失败。相反，这由`<batch:commit>`自动处理。

本文档说明如何利用自己的连接器来利用 link:/mule-user-guide/v/3.7/batch-processing[批量处理]文档中介绍的此功能。
...
本文假定您熟悉 link:/mule-user-guide/v/3.7/batch-processing[批量处理]，您拥有 link:/anypoint-connector-devkit/v/3.9/creating-an-anypoint-connector-project[创建一个连接器项目]并且有 link:/anypoint-connector-devkit/v/3.9/connector-attributes-and-operations[定义了操作]可供使用。
