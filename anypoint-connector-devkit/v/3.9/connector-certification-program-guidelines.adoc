...
| 付费 |在Anypoint Studio的设计时间内，可以在Mule应用程序中使用付费连接器。在部署中，连接器不会在没有许可证的情况下运行。客户需要直接从合作伙伴处购买连接器，并且应该使用 link:/anypoint-connector-devkit/v/3.9/certified-connector-license-management[生成的许可证密钥由MuleSoft提供]管理付费连接器的使用。连接器的价格将由合作伙伴决定。
|===
