...
*Important:*要构建连接器项目，您必须先将 link:/anypoint-connector-devkit/v/3.9/setting-up-your-dev-environment[Anypoint连接器DevKit插件]安装到Anypoint Studio中。
...
| `restprocessor`  | *@RestCall processor deprecated from 3.9.0 onward.*创建Anypoint连接器项目时，基于REST的连接器会插入@RestCall参数。
|===
...
*  **NEXT STEP:**继续使用您的API的 link:/anypoint-connector-devkit/v/3.9/authentication[选择并实施您的验证]方法。
