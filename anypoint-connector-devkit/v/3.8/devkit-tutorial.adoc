...
DevKit可帮助您构建 link:/mule-user-guide/v/3.8/anypoint-connectors[Anypoint连接器]。本教程将向您展示如何编写连接器以支持身份验证，连接，服务操作等。这是连接器开发人员向连接器的最终用户公开的功能，他是Mule应用程序开发人员。

== 什么是Anypoint连接器DevKit？

link:/anypoint-connector-devkit/v/3.8/index[Anypoint连接器DevKit]是Anypoint平台中的一个重要开发工具，可使编码Anypoint连接器变得更加简单。 DevKit是一款基于Maven的工具，可让您构建可作为Mule应用程序的一部分运行的可重用组件，并可从 http://www.mulesoft.com/platform/mule-studio[Anypoint Studio]轻松进行配置。

DevKit向连接器开发人员公开 http://docs.oracle.com/javase/tutorial/java/annotations/[Java注释]，生成代码和文件以与Mule运行时以及Anypoint Studio进行交互。生成的代码提供了连接器和Mule之间的接口，否则这些接口要求每个连接器开发人员都包含大量样板代码，以及在Anypoint Studio中与连接器交互所需的代码和文件。
...
在开发连接器之前，您应该对Mule， link:/anypoint-studio/v/6/download-and-launch-anypoint-studio[Anypoint Studio]和Java开发有一定的了解，特别是使用https://docs.oracle.com/javase/tutorial/java/annotations/basics.html [Java注释]。

[NOTE]
====
有关软件设置和DevKit的信息，请参阅 link:/anypoint-connector-devkit/v/3.8/setting-up-your-dev-environment[设置你的开发环境]和 link:/anypoint-connector-devkit/v/3.8/index[Anypoint连接器DevKit]。
...
. 运行 link:/anypoint-connector-devkit/v/3.8/creating-a-java-sdk-based-connector#generating-sources[生成来源]操作。
+
...
[NOTE]
使用DevKit 3.8构建的连接器支持不同格式的示例规范。请参阅https://docs.mulesoft.com/anypoint-connector-devkit/v/3.8/connector-reference-documentation[Connector参考文档]中的新规范和生成格式。
...
. 在Studio中创建一个Mule应用程序并添加一个HTTP侦听器，并为该路径指定`/get-recently`。
...
* 完整的 link:/anypoint-connector-devkit/v/3.8/developing-devkit-connector-tests[测试文档]。
...
* 请参阅使用DevKit设置 link:/anypoint-connector-devkit/v/3.8/authentication[认证]的文档。
...
具有创建，更新，获取和删除操作的连接器的完整源代码可用https://github.com/mulesoft/mule-cookbook-tutorial/blob/master/src/main/asciidoc/resources/java/v4 /CookbookConnector.java[here。
...
image::pagination-example.png[宽度= “600”]
...
*  link:/anypoint-connector-devkit/v/3.8/authentication[认证]文档。
...
Mule App：
+
image::source/source-debug.png[宽度= “600”]
+
XML：
+
[source,xml,linenums]
...
这里我们明确地使用它来将变换食谱映射到Map中。

Mule App：

image::transformer/transformer-explicit.png[宽度= “600”]
...
. 安装连接器。
+
. 在你的Mule应用程序中，创建一个流，它接收列表中的第一项并在其后添加更新操作。不要使用变压器。
+
image::transformer/transformer-implicitly.png[宽度= “600”]
+
XML：
+
[source,xml,linenums]
...
如果您有wsdl，构建连接器的最简单方法是使用CXF http://cxf.apache.org/docs/wsdl-to-java.html[WSDL2Java的]创建客户端。

您可以非常轻松地在您的`pom.xml`文件中配置CXF目标。完整的文档位于Apache http://cxf.apache.org/docs/maven-cxf-codegen-plugin-wsdl-to-java.html[CXF网站]。
...
----
<1>客户端已准备好向URL `+http://example.com/rest+`发送请求。
<2>为`+http://example.com/rest/resource/helloworld+`添加路径。
<3>以`+http://example.com/rest/resource/helloworld?greeting=Hi+World%21+`的形式配置查询参数。
<4>指定我们希望以JSON格式回复。
<5>发出GET请求。
...
有关更多信息，请参阅https://jersey.github.io/ [Jersey客户端]文档。
